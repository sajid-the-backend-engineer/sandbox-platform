...
func main() {
	...
	// Create a test file
	testContent := []byte("Hello, Northrays!\nThis is a test file.")
	...
	uploadErr := sandbox.FileSystem.UploadFileStream(
		...
		streamedPath,
		northrays.WithUploadProgress(func(p northrays.UploadProgress) {
	...
	stream, err := sandbox.FileSystem.DownloadFileStream(
		ctx,
		testPath,
		northrays.WithDownloadProgress(func(p northrays.DownloadProgress) {
