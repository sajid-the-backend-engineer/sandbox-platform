...
import z from 'zod'
import { getNorthraysClient, getSandboxById, createFileUploadFormat, normalizeSandboxPath } from './utils'
import { Sandbox, CreateSandboxBaseParams, CodeRunParams, NorthraysNotFoundError, CodeLanguage } from '@northrays/sdk'
...
export const createSandbox = createTool({
  ...
  execute: async ({ name, labels, language, envVars }) => {
    ...
    try {
      ...
      const sandbox: Sandbox = await northrays.create(sandboxParams)
  ...
  execute: async ({ sandboxId, path }, context) => {
    try {
      ...
      try {
        ...
        if (e instanceof NorthraysNotFoundError)
          return {
            exists: false,
            path: normalizedPath,
          }
