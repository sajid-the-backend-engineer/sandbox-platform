...
// Represents an Amp Code session within a Northrays sandbox
export class AmpSession {
  ...
  private async runAmpCommand(args: string[]): Promise<void> {
    ...
    await this.ptyHandle!.sendInput(`cd /home/northrays && ${command}\n`)
  ...
  private async getThreadIdFromList(): Promise<string | null> {
    const result = await this.sandbox.process.executeCommand('amp threads list', '/home/northrays')
