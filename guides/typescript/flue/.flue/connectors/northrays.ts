 ...
 */
/**
 * Northrays connector for Flue.
 *
 * Wraps an already-initialized Northrays sandbox into Flue's SandboxFactory
 * interface. The user creates and configures the sandbox using the Northrays
 ...
 *
 * const client = new Northrays({ apiKey: process.env.NORTHRAYS_API_KEY });
 * const sandbox = await client.create({ image: 'ubuntu:latest' });
 * const agent = await init({ sandbox: northrays(sandbox), model: 'anthropic/claude-sonnet-4-6' });
...
import type { Sandbox as NorthraysSandbox } from '@northrays/sdk'

export interface NorthraysConnectorOptions {
  /**
...
/**
 * Implements SandboxApi by wrapping Northrays's TypeScript SDK.
 */
class NorthraysSandboxApi implements SandboxApi {
  constructor(private sandbox: NorthraysSandbox) {}
...
/**
 * Create a Flue sandbox factory from an initialized Northrays sandbox.
 * The user owns the sandbox lifecycle; Flue wraps it into a SessionEnv
 * for agent use.
 */
export function northrays(sandbox: NorthraysSandbox, options?: NorthraysConnectorOptions): SandboxFactory {
  return {
    async createSessionEnv({ cwd }: { id: string; cwd?: string }): Promise<SessionEnv> {
      const sandboxCwd = cwd ?? (await sandbox.getWorkDir()) ?? '/home/northrays'
      const api = new NorthraysSandboxApi(sandbox)
      ...
      if (options?.cleanup === true) {
        cleanupFn = async () => {
          try {
            ...
            console.error('[flue:northrays] Failed to delete sandbox:', err)
          }
        }
