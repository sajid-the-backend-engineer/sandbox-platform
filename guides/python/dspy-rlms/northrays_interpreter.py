...
# =============================================================================
# NorthraysInterpreter - Implements CodeInterpreter protocol using Northrays
# =============================================================================

# Python setup code injected into the Northrays context.
...
class NorthraysInterpreter:
    """Remote interpreter using Northrays sandbox for Python execution.

    Implements the CodeInterpreter protocol for secure code execution in a
    Northrays cloud sandbox. The sandbox provides isolated execution with
    persistent state across calls.

    Prerequisites:
        - Northrays SDK: pip install northrays
        - NORTHRAYS_API_KEY environment variable set

    Example:
        ```python
        # Basic execution
        with NorthraysInterpreter() as interp:
    ...
    def __init__(
        ...
        Args:
            ...
            sandbox_params: Optional parameters to pass to Northrays sandbox creation.
        ...
        self._broker_url = None  # Preview URL for broker (e.g., https://xxx.preview.northrays.work)
    ...
    def start(self) -> None:
        """Initialize the Northrays sandbox and interpreter context."""
        if self._initialized:
            return

        try:
            from northrays import Northrays, SessionExecuteRequest  # pylint: disable=import-outside-toplevel

            self._session_execute_request = SessionExecuteRequest
        except ImportError as exc:
            raise CodeInterpreterError(
                "Northrays not installed. Install with: pip install northrays\n"
                "Also ensure NORTHRAYS_API_KEY environment variable is set."
            ) from exc

        logger.info("Creating Northrays sandbox...")
        self._northrays = Northrays()
        self._sandbox = self._northrays.create(**self.sandbox_params)
...
with open("/home/northrays/broker_server.py", "w") as f:
    f.write(broker_code)
...
"""
        ...
        self._sandbox.process.execute_session_command(
            ...
            self._session_execute_request(
                command="cd /home/northrays && python broker_server.py",
                run_async=True,
            ),
        )
    ...
    def _wait_for_broker_health(self, timeout: float = 30.0) -> None:
        ...
        headers = {"x-northrays-preview-token": self._broker_token}
    ...
    def execute(
        ...
        """Execute Python code in the Northrays sandbox.
    ...
    def shutdown(self) -> None:
        """Release resources and delete the Northrays sandbox."""
