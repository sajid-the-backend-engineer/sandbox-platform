...
async def connect_to_env(url: str, ping_timeout: int = WS_PING_TIMEOUT) -> FinQAEnv:
    """Create a FinQAEnv, connect, and set a long ping timeout for vLLM."""
    env = FinQAEnv(base_url=url)
    await env.connect()
    if hasattr(env, "_ws") and env._ws is not None:
        env._ws.ping_timeout = ping_timeout
    return env
    ...
    async def connect_one(i: int, url: str):
        async with play_sem:
            for attempt in range(MAX_PLAY_RETRIES):
                try:
                    envs[i] = await connect_to_env(url)
                    return
...
async def train(args):
    ...
    if not debug_logger.handlers:
        debug_handler = logging.FileHandler(os.path.join(args.run_dir, "debug.log"), mode="a")
    ...
    if n_gpus < tp_size + 1:
        raise RuntimeError(f"Need at least {tp_size + 1} GPUs (TP={tp_size} + 1 for training), found {n_gpus}")
