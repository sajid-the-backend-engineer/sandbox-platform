# Serving vLLM on GPU Sandboxes (vLLM + Northrays)

## Overview

This guide demonstrates how to serve an open-weights model with [vLLM](https://github.com/vllm-project/vllm) on a Northrays GPU sandbox and query it from anywhere through a token-authenticated preview URL. The server speaks the OpenAI-compatible API, so any OpenAI client works against it unchanged.
...
- **Northrays:** GPU sandboxes are currently experimental, make sure your organization has access to GPUs
