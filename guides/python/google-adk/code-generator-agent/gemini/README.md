# Code Generator Agent Example (Google ADK + Northrays)

## Overview

This example demonstrates how to build a [Google ADK](https://google.github.io/adk-docs/) agent that generates and verifies code using [Northrays](https://northrays.com) sandboxes. The agent uses the `NorthraysPlugin` to execute code in an isolated environment, enabling automated code generation workflows with built-in testing.
...
- **Secure sandbox execution:** All code runs in isolated Northrays sandboxes
...
2. Install dependencies:
   ...
   ```bash
   pip install -U google-adk northrays-adk python-dotenv
   ```
...
4. **Execute in Sandbox:** Code and tests run in an isolated Northrays sandbox
...
For the complete API reference, see the [northrays-adk documentation](https://github.com/northrays/sandbox-platform-adk-plugin#available-tools).
...
- [Northrays ADK Plugin](https://pypi.org/project/northrays-adk/)
