...
exports.onExecutePostLogin = async (event, api) => {
  ...
  // Deny access if this user doesn't exist in the Northrays database
  try {
    const response = await fetch(`${event.secrets.NORTHRAYS_API_URL}/users/${event.user.user_id}`, {
      headers: {
        Authorization: `Bearer ${event.secrets.NORTHRAYS_API_KEY}`,
      },
    })
    ...
    console.error('Failed to fetch Northrays users:', error)
    return api.access.deny('Something went wrong, please try again later')
  }
}
