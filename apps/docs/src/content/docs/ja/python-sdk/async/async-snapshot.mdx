...
Northrays（サンドボックスの作成・管理を行うプラットフォーム）における、事前構成済みのサンドボックスであるスナップショット（リソースや依存関係を含む再利用可能なイメージ/テンプレート）を表します。
...
Northraysのスナップショット（スナップショット（リソースや依存関係を含む再利用可能なイメージ/テンプレート））を管理するサービス。スナップショットの一覧、取得、作成、削除に使用できます。
...
async with AsyncNorthrays() as northrays:
    snapshots = await northrays.snapshot.list()
...
- `image` _Union[str, Image]_ - スナップショットのイメージ。文字列が指定された場合は、
  ...
  Northraysで新しいイメージを作成する際に使用されます。
