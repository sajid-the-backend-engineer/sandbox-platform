...
事前に構成されたサンドボックスである Northrays のスナップショットを表します。
...
Northraysのスナップショットを管理するサービス。スナップショットの一覧、取得、作成、削除に使用できます。
...
```python
northrays = Northrays()
snapshots = northrays.snapshot.list()
...
- `image` _Union[str, Image]_ - スナップショットのイメージ。文字列が指定された場合は、
  ...
  Northrays で新しいイメージを作成するために使用されます。
