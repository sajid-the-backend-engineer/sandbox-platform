...
スナップショット（サンドボックス作成に使う再利用可能な事前設定イメージ／テンプレート）は、Northrays のサンドボックス（Northrays が管理する隔離された一時的な計算環境）に必要な依存関係、ツール、環境設定、リソース要件をすべて含む事前構成済みテンプレートです。Northrays は、標準的な [Docker](https://www.docker.com/) または [OCI](https://opencontainers.org/) 互換イメージからのスナップショット作成をサポートします。

## スナップショットの作成

サンドボックスを起動する際、Northrays は `python`、`node`、`pip` ならびに一般的な pip パッケージなどの有用なユーティリティがプリインストールされたシンプルなイメージに基づくスナップショットを使用します。詳細は[以下](#default-snapshot)を参照してください。

この挙動は上書き可能です。ダッシュボードにアクセスし、[Snapshots](https://app.northrays.com/dashboard/snapshots) をクリックしてから `Create Snapshot` を選択すると、カスタムスナップショットを作成できます。
...
entrypoint フィールドは任意です。イメージに長時間稼働する entrypoint がない場合、Northrays は自動的に `sleep infinity` を実行して、作成直後にコンテナが即時終了しないようにします。
...
スナップショットには Northrays サンドボックスのリソース要件が含まれます。デフォルトでは、Northrays サンドボックスは **1 vCPU**、**1GB RAM**、**3GiB ディスク**を備えています。

より高いパフォーマンスが必要ですか？`Resources` クラスを使用して、必要な CPU、メモリ、ディスク容量を正確に指定できます。

利用可能なリソースと上限は[ダッシュボード](https://app.northrays.com/dashboard/limits)で確認してください。
...
:::note
すべてのリソースパラメータは任意です。指定しない場合、Northraysはデフォルト値を使用します。
:::
...
1. ダッシュボードの[Registries](https://app.northrays.com/dashboard/registries)ページに移動
2. `Add Registry`ボタンをクリック
3. 適切なカスタム名、URL、ユーザー名、パスワード、プロジェクト（該当する場合）を入力
4. コンテナレジストリを作成したら、[Snapshots](https://app.northrays.com/dashboard/snapshots)ページに戻る
...
プライベートコンテナレジストリを手動で用意してイメージをプッシュする手間を避けるために、[Northrays CLI](/docs/ja/getting-started#cli)を使ってローカルイメージやローカルの Dockerfile からスナップショットを作成し、サンドボックスで使用できます。

`docker images` を実行して使用したいイメージとタグが存在することを確認したら、`northrays snapshot push <your_local_docker_image>` コマンドでスナップショットを作成して Northrays にプッシュします。例:
...
たとえば `northrays snapshot push custom-alpine:3.21 --name alpine-minimal --cpu 2 --memory 4 --disk 8` は、2 vCPU、4GiB メモリ、8GiB ディスク容量の Alpine サンドボックスを作成します。
:::

:::note
Northrays はローカルイメージが AMD64 アーキテクチャ向けにビルドされていることを前提としています。したがって、お使いのマシンが別のアーキテクチャの場合は、Docker イメージをビルドする際に `--platform=linux/amd64` フラグが必要です。

詳しくは [CLI ドキュメント](/docs/ja/tools/cli#northrays-snapshot-push) を参照してください。
:::
...
また、CLI で実行する場合は、`create` で `--dockerfile` フラグに使用したい Dockerfile のパスを渡すと、Northrays がスナップショット（Snapshot）をビルドします:

```bash
northrays snapshot create data-analysis01 --dockerfile ./Dockerfile --context ./requirements.txt
```
...
...
 ...
 ✓  Use 'harbor-transient.internal.northrays.app/northrays/trying-northrays:0.0.1' to create a new sandbox using this Snapshot

```

## スナップショットの削除

作成したカスタムスナップショットの削除は簡単です。[Snapshots](https://app.northrays.com/dashboard/snapshots) ページに移動し、削除したいスナップショットの行末にある三点アイコンをクリックして表示される `Delete` ボタンを選択してください。
...
Northraysのサンドボックスは内部でDockerコンテナを実行できます（Docker-in-Docker）。これにより、コンテナ化されたアプリケーションのビルド、テスト、デプロイが可能になります。特に、データベース、メッセージキュー、その他のマイクロサービスなど外部サービスに依存するプロジェクトで有用です。
...
Northrays が使用するデフォルトのスナップショットは、`python` と `node`、それぞれの LSP に加えて、以下の `pip` 事前インストール済みパッケージを含むイメージをベースにしています:
