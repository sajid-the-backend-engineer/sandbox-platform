...
Northrays スナップショットを管理するためのサービス。スナップショットの一覧表示、取得、作成、削除に利用できます。
...
```ts
const northrays = new Northrays();
const snapshots = await northrays.snapshot.list();
...
あらかじめ構成されたサンドボックスである Northrays のスナップショットを表します。
