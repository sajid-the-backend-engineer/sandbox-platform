---
title: Northrays MCPサーバー
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

Northrays Model Context Protocol (MCP) サーバーは、AIエージェントがNorthraysの機能にプログラムからアクセスし、操作できるようにします。本ガイドでは、各種AIエージェントでMCPサーバーをセットアップして利用する方法を解説します。
...
使用するAIエージェントで Northrays の MCPサーバーを初期化します:
...
初期化後、AIエージェントのアプリケーションを起動して、Northrays の機能の利用を開始します。

## 他のAIエージェントとの統合

Northrays MCP を他のAIエージェントと統合するには、次の手順に従ってください。
...
- **サンドボックスの破棄**
  - 既存の Northrays サンドボックスを削除します

### ファイル操作

- **ファイルのダウンロード**

  - Northrays サンドボックスからファイルをダウンロードします
...
- **ファイルのアップロード**
  ...
  - Northrays サンドボックスにファイルをアップロードします
...
- **認証の問題**
  ...
  - 資格情報を更新するために `northrays login` を実行する
...
- **サンドボックスのエラー**
  - `northrays sandbox list` を使用してサンドボックスのステータスを確認する

## サポート

サポートが必要な場合は、次をご利用ください:

- [northrays.com](https://northrays.com) をご覧ください
- support@northrays.com までお問い合わせください
