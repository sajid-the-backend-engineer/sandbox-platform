---
title: Limits
description: Overview of your organization's resource limits and usage.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

[Northrays Limits ↗](https://app.northrays.com/dashboard/limits) provide an overview of your organization's [current usage](#current-usage), [resources](#resources), [sandbox limits](#sandbox-limits), and [rate limits](#rate-limits). Northrays uses a [tier-based](#tiers) system where organizations are placed into tiers based on verification status, with each tier providing access to a specific compute pool and rate limits. For information on spending and wallet management, see [billing](/docs/en/billing).
...
Northrays supports managing your sandbox resources by [changing the sandbox state](/docs/sandboxes#sandbox-lifecycle). The table below summarizes how each state affects sandbox resource usage:
...
Northrays includes rate limit information in API response headers. Header names include a suffix based on which rate limit is triggered (e.g., `-anonymous`, `-authenticated`, `-sandbox-create`, `-sandbox-lifecycle`):
...
Northrays [Python](/docs/python-sdk), [TypeScript](/docs/en/typescript-sdk), [Ruby](/docs/ruby-sdk) and [Go](/docs/go-sdk) SDKs raise or throw a `NorthraysRateLimitError` exception (Python) or error (TypeScript, Ruby and Go) when you exceed a rate limit.
...
<Tabs>
  ...
  <TabItem label="Python" icon="seti:python">
    ```python
    try:
      northrays.create(snapshot="my-snapshot")
    except NorthraysRateLimitError as e:
  ...
  <TabItem label="Go" icon="seti:go">
    ```go
    sandbox, err := northrays.Create(ctx, nil)
    if err != nil {
      var rateLimitErr *errors.NorthraysRateLimitError
...
Limits are applied to your organization's default region. To unlock higher limits, complete the following verification steps in the [Northrays Dashboard ↗](https://app.northrays.com/dashboard/limits):
...
| Custom   | Custom                               | Contact [support@northrays.com](mailto:support@northrays.com) |
...
async function createSandboxWithRetry() {
  ...
  while (retries < maxRetries) {
    try {
      return await northrays.create({ snapshot: 'my-snapshot' })
    } catch (error) {
      if (error instanceof NorthraysRateLimitError && retries < maxRetries - 1) {
