...
description: Use Northrays to run AI-generated code for data analysis and visualization.
---
...
You can use Northrays Sandbox to run AI-generated code to analyze data. Here's how the AI data analysis workflow typically looks:
...
## Build an AI Data Analyst with Northrays

This example shows how to build an AI-powered data analyst that automatically generates insights and visualizations from CSV data using Northrays's secure sandbox environment.
...
Install the Northrays SDK and Anthropic SDK to your project:

<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    `bash pip install northrays anthropic python-dotenv `
  </TabItem>

  <TabItem label="TypeScript" icon="seti:typescript">
    `bash npm install @northrays/sdk @anthropic-ai/sdk dotenv `
  </TabItem>

  <TabItem label="Ruby" icon="seti:ruby">
    `bash gem install northrays anthropic dotenv `
  </TabItem>
</Tabs>
...
Now create a [Northrays sandbox](/docs/en/sandboxes/#create-sandboxes) and upload your dataset:
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    northrays = Northrays() # The sandbox language is Python by default.
    sandbox = northrays.create()

    # Upload the dataset to the sandbox

    sandbox.fs.upload_file("dataset.csv", "/home/northrays/dataset.csv")
  ...
  <TabItem label="TypeScript" icon="seti:typescript">
    ...
    // Create sandbox
    const northrays = new Northrays(); // The sandbox language is Python by default.
    const sandbox = await northrays.create()

    // Upload the dataset to the sandbox
    await sandbox.fs.uploadFile('dataset.csv', '/home/northrays/dataset.csv')
    ```
  </TabItem>
  ...
  <TabItem label="Ruby" icon="seti:ruby">
    ...
    # Create sandbox
    northrays = Northrays::Northrays.new # The sandbox language is Python by default.
    sandbox = northrays.create

    # Upload the dataset to the sandbox
    sandbox.fs.upload_file(File.read('dataset.csv'), '/home/northrays/dataset.csv')
    ```
  </TabItem>
</Tabs>
...
Now we'll create the core functionality that connects Claude with Northrays to analyze data and generate visualizations.
...
<Tabs syncKey="language">
  <TabItem label="Python" icon="seti:python">
    ...
    prompt = f"""
    I have a CSV file with vehicle valuations saved in the sandbox at /home/northrays/dataset.csv.
...
- **Secure execution:** Code runs in isolated Northrays sandboxes
...
- **Language agnostic:** While we used Python here, Northrays supports multiple languages
