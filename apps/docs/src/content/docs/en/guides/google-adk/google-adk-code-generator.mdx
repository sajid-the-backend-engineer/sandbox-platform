...
description: Build Google ADK agents that generate and test code using Northrays's isolated sandbox environment.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide demonstrates how to use the `NorthraysPlugin` for Google ADK to build an agent that generates, tests, and verifies code in a secure sandbox environment. The plugin enables agents to execute Python, JavaScript, and TypeScript code, run shell commands, and manage files within isolated Northrays sandboxes.
...
You describe the function you want in plain English, specifying the language (Python, JavaScript, or TypeScript). The agent generates the implementation, writes tests for it, and executes everything in a Northrays sandbox. If tests fail, the agent automatically fixes the code and re-runs until all tests pass. Only then does it return the verified, working code.
...
Clone the Northrays repository and navigate to the example directory:

```bash
git clone https://github.com/northrays/sandbox-platform
cd northrays/guides/python/google-adk/code-generator-agent/gemini
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    - `northrays-adk`: Provides the `NorthraysPlugin` that enables secure code execution in Northrays sandboxes
...
1. **Northrays API key:** Get it from [Northrays Dashboard](https://app.northrays.com/dashboard/keys)
...
The `NorthraysPlugin` provides tools that allow the agent to:
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    from northrays_adk import NorthraysPlugin
    ...
    **What each import does:**
    ...
    - `NorthraysPlugin`: Provides sandbox execution tools to the agent
    ...
    **Logging configuration:**
    ...
    - `logging.DEBUG`: Most verbose, shows all internal operations including NorthraysPlugin sandbox creation and tool invocations
    ...
    With `DEBUG` level logging enabled, you can see the NorthraysPlugin's internal operations, including when the sandbox is created, when the `execute_code_in_northrays` tool is invoked, and when cleanup occurs. The plugin's `plugin_name` (configurable, defaults to `northrays_plugin`) appears in these log messages, making it easy to trace plugin activity.
    :::
  </TabItem>
</Tabs>
...
### 7. Configure the Northrays Plugin
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    **Configuration options:**
    ...
    - `api_key`: Northrays API key (defaults to `NORTHRAYS_API_KEY` env var)
    - `sandbox_name`: Custom name for the sandbox
    - `plugin_name`: Name displayed in logs when the plugin logs messages (defaults to `northrays_plugin`)
...
Create the agent with the Gemini model, our instruction, and the Northrays tools:
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    **Parameters explained:**
    ...
    - `tools`: List of tools from the Northrays plugin that the agent can use
  </TabItem>
</Tabs>
...
- **NorthraysPlugin operations**: Sandbox creation, tool invocations (`execute_code_in_northrays`), and cleanup
- **LLM requests and responses**: The prompts sent to Gemini and the responses received
- **Plugin registration**: Confirmation that the `northrays_plugin` was registered with the agent
...
```
DEBUG:northrays_adk.plugin:Northrays sandbox created: e38f8574-48ac-48f1-a0ff-d922d02b0fcb
INFO:google_adk.google.adk.plugins.plugin_manager:Plugin 'northrays_plugin' registered.
```

The NorthraysPlugin creates an isolated sandbox and registers itself with the agent.
...
The agent writes both the implementation and test cases, then calls the `execute_code_in_northrays` tool:
...
-----------------------------------------------------------
Function calls:
name: execute_code_in_northrays, args: {'code': "...", 'language': 'typescript'}
```

**Step 4: Code execution in sandbox**

```
DEBUG:northrays_adk.plugin:Before tool: execute_code_in_northrays
DEBUG:northrays_adk.tools:Executing typescript code (length: 1570 chars)
DEBUG:northrays_adk.tools:Code execution completed with exit_code: 0
DEBUG:northrays_adk.plugin:After tool: execute_code_in_northrays
```
...
```
INFO:northrays_adk.plugin:Deleting Northrays sandbox...
INFO:northrays_adk.plugin:Northrays sandbox deleted.
...
- **Secure execution:** Code runs in isolated Northrays sandboxes, not on your machine
...
For the complete API reference of the Northrays ADK plugin, including all available tools and configuration options, see the [northrays-adk documentation](https://github.com/northrays/sandbox-platform-adk-plugin#available-tools).
