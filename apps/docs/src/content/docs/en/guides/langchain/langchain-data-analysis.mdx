...
description: Build LangChain agents that perform secure data analysis using Northrays's isolated sandbox environment.
---
...
This package provides the `NorthraysDataAnalysisTool` - LangChain tool integration that enables agents to perform secure Python data analysis in a sandboxed environment. It supports multi-step workflows, file uploads/downloads, and custom result handling, making it ideal for automating data analysis tasks with LangChain agents.
...
You upload your dataset and provide a natural language prompt describing the analysis you want. The agent reasons about your request, determines how to use the `NorthraysDataAnalysisTool` to perform the task on your dataset, and executes the analysis securely in a Northrays sandbox.
...
Clone the [repository](https://github.com/northrays/sandbox-platform) and navigate to the example directory:

```bash
git clone https://github.com/northrays/sandbox-platform.git
cd northrays/guides/python/langchain/data-analysis/anthropic
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    - `langchain-northrays-data-analysis`: Provides the `NorthraysDataAnalysisTool` for LangChain agents
...
[https://download.northrays.com/dataset.csv](https://download.northrays.com/dataset.csv)
...
Now we'll initialize the `NorthraysDataAnalysisTool` and upload our dataset.
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    DataAnalysisTool = NorthraysDataAnalysisTool(
...
<Tabs>
  ...
  **What happens here:**
    ...
    2. It determines it needs to use the `NorthraysDataAnalysisTool`
    3. Agent generates Python code to analyze the data
    4. Code executes securely in the Northrays sandbox
...
# Load the dataset
df = pd.read_csv('/home/northrays/dataset.csv')
...
**Step 3: Code executes in Northrays sandbox**
...
- **Secure execution:** Code runs in isolated Northrays sandbox
...
The following public methods are available on `NorthraysDataAnalysisTool`:
...
file_bytes = tool.download_file("/home/northrays/results.csv")
```
...
Uploads a file to the sandbox. The file is placed in `/home/northrays/`.
...
This method provides access to the Northrays sandbox instance, allowing you to inspect sandbox properties and metadata, as well as perform any sandbox-related operations. For details on available attributes and methods, see the [Sandbox](#sandbox) data structure section below.
...
:::note
The list of preinstalled packages in a sandbox can be found at [Northrays's Default Snapshot documentation](https://www.northrays.com/docs/en/snapshots/#default-snapshots).
:::
...
See the full structure and API in the [Northrays Python SDK Sandbox documentation](https://www.northrays.com/docs/en/python-sdk/sync/sandbox/#sandbox).