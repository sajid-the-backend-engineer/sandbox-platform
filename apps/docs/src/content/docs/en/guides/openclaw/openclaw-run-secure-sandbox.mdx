---
title: Run OpenClaw in a Northrays Sandbox via CLI
description: Learn how to run OpenClaw inside a Northrays sandbox using the Northrays CLI for secure, isolated, and always-on AI assistant with Telegram and WhatsApp.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

This guide walks you through setting up [OpenClaw](https://openclaw.ai/) inside a Northrays sandbox and configuring Telegram and WhatsApp channels.

Running OpenClaw in a Northrays sandbox keeps your AI assistant isolated from your local machine, provides a secure environment for code execution, and ensures your bot stays online 24/7 without tying up your personal computer.
...
```bash
northrays sandbox create --name openclaw --snapshot northrays-medium --auto-stop 0
```

OpenClaw comes preinstalled in the default Northrays snapshot, so the command above is all you need.

:::note
The `--auto-stop 0` flag disables automatic shutdown, keeping OpenClaw accessible until you manually stop or delete the sandbox. The `northrays-medium` snapshot is required because the OpenClaw gateway needs a minimum of 2GB memory.
:::
...
The OpenClaw dashboard is a web interface for managing your assistant, monitoring connections, and configuring channels. To access it, you need a [preview URL](https://www.northrays.com/docs/en/preview/) that exposes the gateway port running inside your sandbox.

In your local terminal (not inside the sandbox SSH session), generate the preview URL:

```bash
northrays preview-url openclaw --port 18789
```

This command generates a [signed preview URL](https://www.northrays.com/docs/en/preview/#signed-preview-url) that securely exposes the port.
