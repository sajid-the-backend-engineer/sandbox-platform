---
title: Multi-Turn RL Training with OpenEnv and Northrays
description: Use parallel Northrays sandboxes with OpenEnv's FinQA environment to train Qwen3-14B on financial question answering via GRPO.
---
...
cd northrays/guides/python/reinforcement-learning/openenv
```
...
- `northrays` - Northrays SDK for sandbox management
...
Get your Northrays API key from the [Northrays Dashboard](https://app.northrays.com/dashboard/keys) and create a `.env` file:
...
Before running any episodes, build a Northrays snapshot that pre-packages the FinQA environment server and dataset:
...
This uses Northrays's declarative Image API to clone the FinQA environment from the OpenEnv repo, install its dependencies, and pre-download the FinQA dataset from HuggingFace, all baked into a single container image for fast sandbox startup.
...
The `run.py` script demonstrates the full OpenEnv + Northrays integration in a single episode. Run it with:
...
The `NorthraysProvider` from OpenEnv wraps the Northrays SDK, creating a sandbox from the pre-built snapshot and waiting for the FinQA server to become healthy:

<Tabs>
  <TabItem label="Python" icon="seti:python">
    ```python
    from openenv.core.containers.runtime.northrays_provider import NorthraysProvider
...
<Image
  src={rewardsPlot}
  alt="Episode success rate per iteration, rising from ~21% to ~52%"
  width={700}
...
| -------------------------- | ---------------- | ---------------------------------------------- |
| `--sandboxes`              | 500              | Number of concurrent Northrays sandboxes         |
...
| `--snapshot`               | `openenv-finqa`  | Northrays snapshot name                          |
