---
title: Scaling Tool Execution in RL Training with veRL and Northrays
description: Run veRL's ReTool recipe on Northrays sandboxes, configure the Northrays backend, and benchmark tool-calling throughput.
---

import { Image } from 'astro:assets'

import northraysThroughputPlot from '../../../../../assets/docs/images/verl-retool-northrays-throughput.svg'
import verlToolFigure from '../../../../../assets/docs/images/verltool-figure-2-table-2.png'

This guide demonstrates how to run [veRL's](https://github.com/verl-project/verl) ReTool recipe with Northrays sandboxes as the tool execution backend, scaling up to hundreds of tool calls per training step without hitting a concurrency ceiling.
...
### 3. Northrays as the ReTool Backend

By executing tool calls on Northrays sandboxes, the async rollout pipeline can scale to hundreds of concurrent executions without saturating the backend.
...
- **Async SDK.** The `AsyncNorthrays` client integrates directly with veRL's async rollout workers. Workers fire requests in parallel and process results as they arrive.
...
The chart below compares code execution throughput between Docker containers and Northrays sandboxes.

<Image
  src={northraysThroughputPlot}
  alt="Line chart comparing throughput between Docker containers and Northrays as concurrency increases from 1 to 128."
  width={900}
  style="max-width: 100%; height: auto; margin: 1rem 0;"
/>

With Docker containers, throughput plateaus as concurrency increases. Container startup overhead dominates, and adding more parallelism doesn't help. Northrays sandboxes scale linearly and reach **98 calls/sec** at 128 concurrent — a **5.5x throughput improvement** at peak concurrency.
...
#### Export the Northrays API Key

Get your API key from the [Northrays Dashboard](https://app.northrays.com/dashboard/keys) and export it before running the recipe or the benchmark:

```bash
export NORTHRAYS_API_KEY="your_northrays_api_key"
```

### 5. Start Training

Use the existing ReTool launch script and point it at the Northrays tool config and the downloaded checkpoint:

```bash
TOOL_CFG=recipe/retool/northrays_tool_config.yaml
...
bash recipe/retool/run_qwen2-32b_dapo.sh \
  ...
  trainer.project_name=retool_northrays \
  trainer.experiment_name=qwen2.5-32b_dapo_northrays
```
...
cd northrays/guides/python/reinforcement-learning/verl-retool
...
# Northrays sandboxes (requires NORTHRAYS_API_KEY and veRL)
python benchmark_tool_backends.py \
  --backend northrays \
...
Benchmarked on macOS (Docker Desktop) and Northrays cloud (includes network round-trip). Absolute numbers may vary by environment.
