---
title: Run DSPy RLMs on Northrays
description: Use DSPy's RLM module with a Northrays-backed interpreter to safely execute LLM-generated Python code in an isolated cloud sandbox.
---
...
The generated code runs in a REPL, and in this guide we use and present `NorthraysInterpreter`, which plugs into DSPy as the code-execution backend so that all generated code runs inside an isolated Northrays cloud sandbox rather than on your machine.
...
cd northrays/guides/python/dspy-rlms
```
...
This installs the DSPy framework and the Northrays SDK. To also run the included demo (which plots results with matplotlib), use
...
:::note
Get your Northrays API key from the [Northrays Dashboard](https://app.northrays.com/dashboard/keys). The LLM provider key depends on which model you configure in your code — the examples use OpenRouter.
:::

### 2. Basic Usage

The following example shows the basic setup — configure a model, create a `NorthraysInterpreter`, and pass it to an RLM. The generated code can call `llm_query()` to delegate semantic work back to the LLM:
...
Each RLM call runs an iterative REPL loop. The LLM writes Python code, the code executes in a Northrays sandbox, and the output is fed back to the LLM for the next iteration. Crucially, the generated code can call `llm_query()` to invoke a sub-LLM call — this is how the LLM delegates semantic work (understanding, extraction, classification) to itself while keeping the orchestration logic in Python.
...
3. **Execute** — The code runs inside a Northrays sandbox; any `llm_query()` calls are bridged back to the host LLM
...
```
Host Process                                    Northrays Sandbox
┌──────────────────────────────┐                ┌──────────────────────────────┐
│      NorthraysInterpreter      │                │    Broker Server (Flask)     │
│                              │                │                              │
...
- **Isolation** — All generated code runs in a Northrays cloud sandbox, not on your machine
