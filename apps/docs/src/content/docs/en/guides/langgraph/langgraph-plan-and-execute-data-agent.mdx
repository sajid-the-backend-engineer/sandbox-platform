---
title: Build a Plan-and-Execute Data Agent With LangGraph and Northrays
description: Wire a six-node LangGraph state machine that runs an end-to-end ETL plus analytical-SQL workflow in an isolated Northrays sandbox, with structured planning and per-step retry recovery.
---
...
You ask the agent a natural-language data question. The planner emits an ordered list of atomic plan steps as structured output. For each step the executor generates Python code and runs it in a Northrays sandbox. A deterministic `check` node advances to the next step on success, retries the current step (with the failing code as context, up to `max_attempts`) on failure, or routes to the summarizer once the plan is complete. The summarizer produces a final natural-language answer and a `cleanup` node deletes the sandbox.
...
cd northrays/guides/python/langgraph/plan-and-execute-data-agent
```
...
<Tabs>
  <TabItem label="Python" icon="seti:python">
    ...
    - `northrays`: Northrays Python SDK for sandbox provisioning and code execution
...
Before walking through the implementation, here are the key concepts the code relies on: the choice of agent pattern (plan-and-execute vs. ReAct), LangGraph's `StateGraph` with task-specific state, how state updates flow through nodes, structured-output planning, and the Northrays code interpreter.
...
- **Persistent interpreter context:** Northrays's `code_interpreter.run_code` shares one Python interpreter across all `execute` calls, so imports, variables, and functions defined in earlier steps stay in scope, exactly the behavior the LLM's plan assumes.
- **Secure sandbox execution:** Every line of LLM-generated Python runs in an isolated Northrays sandbox, not on your machine.
