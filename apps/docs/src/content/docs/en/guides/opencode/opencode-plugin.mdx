---
title: Run OpenCode with the Northrays Plugin
description: Use the Northrays OpenCode plugin to run each OpenCode session in a Northrays sandbox, with automatic git sync to local branches.
---
...
This guide demonstrates how to run the [Northrays OpenCode plugin](https://www.npmjs.com/package/@northrays/opencode) which integrates Northrays sandboxes and OpenCode. When the plugin is active, all agent operations occur in secure sandboxes, with one sandbox per OpenCode session. The plugin also has the ability to sync changes between sandboxes and local Git branches.

### 1. Workflow Overview

When you run OpenCode with the Northrays plugin, sandboxes are created automatically inside OpenCode sessions. Operations such as running code, installing dependencies, and starting servers occur in the sandbox.

<Image
  src={opencodePluginGif}
  alt="OpenCode with Northrays plugin — sandbox toast and git branches"
  width={600}
...
Add the Northrays plugin to your project by creating or editing `opencode.json` in the project directory:
...
To confirm the plugin is working, type `pwd` in the chat and you should see a path like `/home/northrays/project`.
...
The Northrays plugin consists of several modules that provide custom tools, hooks for events, connections from sessions to sandboxes and system prompt transforms. These modules ensure every agent action runs in a [Northrays sandbox](/docs/en/sandboxes) and that changes sync to local Git branches:
...
The custom tools module registers overrides for OpenCode's built-in tools so that every file and process operation goes through the Northrays SDK. It receives the project `id` and `worktree` from the plugin context and returns a tool map:
...
For example, the plugin implementation of the bash tool uses the Northrays SDK to run the command in the sandbox:
...
For more information on how the sync is implemented, see the [SessionGitManager](https://github.com/northrays/sandbox-platform/blob/main/libs/opencode-plugin/.opencode/plugin/northrays/git/session-git-manager.ts) class.
...
- **macOS:** `~/.local/share/opencode/storage/northrays/[projectid].json`
...
- Secure, isolated execution: each OpenCode session runs in its own Northrays sandbox
