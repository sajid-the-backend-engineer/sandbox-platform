---
title: Build a Generative-UI Coding Agent with CopilotKit and Northrays
description: Wire a CopilotKit Built-in Agent to a Northrays sandbox with full shell and filesystem access, where every tool call streams into the chat as generative UI and any hosted process embeds as a live iframe.
---
...
This guide demonstrates how to build a [CopilotKit](https://docs.showcase.copilotkit.ai/) Built-in Agent backed by a [Northrays](https://www.northrays.com/) sandbox with full shell and filesystem access. The agent handles whatever a developer might do at a terminal: build apps, debug or analyze code, run scripts, work with data, install packages.
...
Clone the [Northrays repository](https://github.com/northrays/sandbox-platform) and navigate to the example directory:
...
- **Northrays API key**, from the [Northrays Dashboard](https://app.northrays.com/dashboard/keys)
...
The agent exposes 11 tools, defined with `defineTool` from `@copilotkit/runtime/v2`. Each is a thin wrapper around a [Northrays TypeScript SDK](https://www.northrays.com/docs/en/typescript-sdk/) call:

| Tool | Northrays SDK call |
|---|---|
| `createSandbox` (with optional `envVars`, `labels`, `autoStopInterval`) | `northrays.create({ public: true, envVars, labels, autoStopInterval })` |
...
The backend pulls the runtime helpers from `@copilotkit/runtime`, the v2 agent + tool helpers from `@copilotkit/runtime/v2`, the Northrays client, the Next.js request type, and Zod for tool parameter schemas:
...
```text
You are a coding agent with shell access to a fresh Northrays sandbox.
...
Work under /home/northrays by default. Reuse the same sandboxId across every
...
2. ALWAYS bind the dev server to 0.0.0.0 or the Northrays proxy will not reach
   it. Cheat sheet:
...
`getPreviewUrl` returns the Northrays proxy URL for any port the sandbox has open. It is the standalone counterpart to `startWebServer`: use it when the agent has already brought up a hosted process by other means (a previous `runCommand`, an already-running service) and just needs the URL to surface as an iframe, without spawning a new dev server:
...
The remaining six tools (`readFile`, `listFiles`, `findFiles`, `searchFiles`, `replaceInFiles`, `getFileDetails`) follow the same shape: a Zod schema for inputs, a one-call wrapper around the Northrays SDK in `execute`, and a structured object returned to the frontend.
...
✓  done
$ cd /home/northrays && npm create vite@latest snake-game -- --template react-ts --yes && cd snake-game && npm install
...
│
◇  Scaffolding project in /home/northrays/snake-game...
│
└  Done. Now run:
...
✓  wrote  /home/northrays/snake-game/src/App.tsx                             4492 B

✓  wrote  /home/northrays/snake-game/src/App.css                             1527 B

✓  wrote  /home/northrays/snake-game/src/index.css                            307 B

✓  wrote  /home/northrays/snake-game/vite.config.ts                           273 B

✓  done
$ cd /home/northrays/snake-game && npm run build
...
●  Live preview
<iframe src="https://5173-a0ffcc3d-7753-4d52-89d0-6c595c60626a.northraysproxy01.net" /> — live preview of the running Snake game

Done — the Snake game is running here: https://5173-a0ffcc3d-7753-4d52-89d0-6c595c60626a.northraysproxy01.net
```

<Image
  src={copilotkitSnakeGame}
  alt="Snake game built and previewed live in the chat by the CopilotKit + Northrays coding agent"
  width={600}
...
```text
✓  wrote  /home/northrays/snake-game/src/App.css                             1529 B
...
- **All code execution happens inside an isolated Northrays sandbox**, never on the host running the chat.
