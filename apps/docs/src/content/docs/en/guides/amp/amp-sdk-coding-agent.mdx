---
title: Build a Coding Agent Using Amp Code and Northrays
description: Step-by-step guide to building an autonomous coding agent using Amp Code CLI and Northrays sandboxes.
---
...
This guide demonstrates how to run an autonomous coding agent based on [Amp Code](https://ampcode.com/) inside a Northrays sandbox environment. The agent can develop full-stack web apps, write code in any language, install dependencies, and run scripts. It can also start and manage dev servers, and generate preview links for live apps.

---

### 1. Workflow Overview

When you launch the main module, a Northrays sandbox is created and the Amp CLI is installed inside it. The agent uses Amp's [streaming JSON mode](https://ampcode.com/manual#cli-streaming-json) for programmatic control.
...
https://8000-29baaaf7-767a-4dff-8129-1e6ec2100b3e.northraysproxy01.net
🔧 create_file /home/northrays/index.html
Successfully created file /home/northrays/index.html
🔧 create_file /home/northrays/start.sh
Running `python3 -m http.server 8000` via session command...

User:
```

The agent can also host web apps and provide you with a preview link using the [Northrays Preview Links](/docs/en/preview/) feature. When your task involves running or previewing a web application, the agent automatically reasons about this need, hosts the app, and generates a preview link for you to inspect the live result:

<Image
  src={ampSdkCodingAgentDemo}
  alt="Amp Code agent creating a Kanji flashcard app in a Northrays sandbox"
  width={600}
...
cd northrays/guides/typescript/amp/amp-sdk
```
...
1. Creates a new [Northrays sandbox](/docs/en/sandboxes) with the Amp API key.
2. Installs the Amp CLI globally in the sandbox.
3. Creates a PTY for streaming output from Amp.
4. Sends a Northrays-aware system prompt as the first user message (preview URL pattern + instruction to write server startup command to `/home/northrays/start.sh`).
...
A Northrays-aware system prompt is sent as the first user message. It instructs the agent to use the preview URL pattern and to write the server start command into `/home/northrays/start.sh` (instead of executing directly in Amp), then provide the preview URL:
...
The readline loop waits for user input, sends it to the agent, and displays the streamed response. If Amp produced `/home/northrays/start.sh`, the script is then launched via Northrays's session command API so long-running/background server startup does not hang Amp turns.
