...
- `NorthraysException` - if the Sandbox fails to start
...
- `NorthraysException` - if start fails or times out
...
- `NorthraysException` - if the Sandbox fails to stop
...
- `NorthraysException` - if stop fails or times out
...
- `NorthraysException` - if timeout is invalid, state becomes error, or timeout expires
...
- `NorthraysException` - if deletion fails
...
- `NorthraysException` - if label update fails
...
- `NorthraysException` - if the update fails
...
- `NorthraysException` - if the request fails
...
- `NorthraysException` - if timeout is invalid, state becomes failure, or timeout expires
...
— including those not returned by `Northrays#list` — are populated.

**Throws**:

- `NorthraysException` - if refresh fails
...
- `NorthraysException` - if the fork operation fails or times out
...
- `NorthraysException` - if the snapshot operation fails
...
- `NorthraysException` - if the pause operation fails
...
- `NorthraysException` - if timeout is negative or the operation fails/times out
...
- `String` - Northrays snapshot used to create this Sandbox, or `null` if none.
...
Not returned by `Northrays#list`; call `#refreshData()` on each item to populate.
