...
- `NorthraysException` - if code is empty, connection fails, or execution fails
...
- `NorthraysException` - if context creation fails
...
- `NorthraysException` - if listing contexts fails
