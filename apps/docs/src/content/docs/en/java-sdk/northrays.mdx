...
Reads `NORTHRAYS_API_KEY`, `NORTHRAYS_API_URL`, and `NORTHRAYS_TARGET`.

**Throws**:

- `NorthraysException` - if required authentication is missing
...
- `config` _NorthraysConfig_ - SDK configuration containing API key and endpoint settings

**Throws**:

- `NorthraysException` - if configuration is invalid or missing credentials
...
- `NorthraysException` - if creation or startup fails
...
- `NorthraysException` - if creation fails or the Sandbox does not start in time
...
- `NorthraysException` - if the Sandbox is not found or request fails
...
`Northrays.list`" on `Sandbox` (env, networkBlockAll, networkAllowList, volumes,
