...
Northrays provides process and code execution capabilities through the `process` module in sandboxes.

## Code execution

Northrays provides methods to execute code in sandboxes. You can run code snippets in multiple languages with support for both stateless execution and stateful interpretation with persistent contexts.
...
Northrays provides methods to run code snippets in sandboxes using stateless execution. Each invocation starts from a clean interpreter, making it ideal for independent code snippets.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/code-run' \
  --request POST \
...
Northrays provides methods to run code with persistent state using the code interpreter. You can maintain variables and imports between calls, create isolated contexts, and control environment variables.

<Tabs syncKey="language">
<TabItem label="Python" icon="seti:python">

```python
from northrays import Northrays, OutputMessage
...
```bash
# Create context
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/interpreter/context' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{}'
...
# wss://proxy.app.northrays.com/toolbox/{sandboxId}/process/interpreter/execute
# Send JSON message:
# {
#   "code": "counter = 1\nprint(f\"Counter initialized at {counter}\")",
...
# Delete context
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/interpreter/context/{contextId}' \
  --request DELETE
```
...
Northrays provides methods to execute shell commands in sandboxes. You can run commands with working directory, timeout, and environment variable options.

The working directory defaults to the sandbox working directory. It uses the WORKDIR specified in the Dockerfile if present, or falls back to the user's home directory if not (e.g., `workspace/repo` implies `/home/northrays/workspace/repo`). You can override it with an absolute path by starting the path with `/`.

### Execute commands

Northrays provides methods to execute shell commands in sandboxes by providing the command string and optional parameters for working directory, timeout, and environment variables. You can also use the `northrays exec` CLI command for quick command execution.
...
response, err = sandbox.Process.ExecuteCommand(ctx, "echo $CUSTOM_SECRET",
	options.WithCommandEnv(map[string]string{"CUSTOM_SECRET": "NORTHRAYS"}),
)
if err != nil {
	log.Fatal(err)
}
...
response = sandbox.process.executeCommand(
    "echo $CUSTOM_SECRET",
    ".",
    Map.of("CUSTOM_SECRET", "NORTHRAYS"),
    null
);
System.out.println(response.getResult());
```
...
```bash
# Execute any shell command
northrays exec my-sandbox -- ls -la

# Setting a working directory and a timeout
northrays exec my-sandbox --cwd workspace/src --timeout 5 -- sleep 3

# Passing environment variables (use shell syntax)
northrays exec my-sandbox -- sh -c 'CUSTOM_SECRET=NORTHRAYS echo $CUSTOM_SECRET'
```
...
Northrays provides methods to manage background process sessions in sandboxes. You can create sessions, execute commands, monitor status, and manage long-running processes.

### Get session status

Northrays provides methods to get session status and list all sessions in a sandbox by providing the session ID.
...
```bash
# Get session info
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/{sessionId}'

# List all sessions
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session'
```
...
Northrays provides methods to retrieve information about the internal entrypoint session in sandboxes. In each sandbox, the configured entrypoint command is executed inside a dedicated internal session, and you can fetch the session details (including the commands) and read its logs.
...
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/entrypoint'

# Get entrypoint logs (HTTP)
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/entrypoint/logs'

# Follow entrypoint logs in real-time (WebSocket)
# wss://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/entrypoint/logs?follow=true
```
...
Northrays provides methods to execute interactive commands in sessions. You can send input to running commands that expect user interaction, such as confirmations or interactive tools like database CLIs and package managers.
...
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/{sessionId}/exec' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "command": "pip uninstall requests",
...
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/{sessionId}/command/{commandId}/input' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "data": "y"
...
# Get command logs
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/process/session/{sessionId}/command/{commandId}/logs'
```
...
Northrays provides methods to manage session resources. You should use sessions for long-running operations, clean up sessions after execution, and handle exceptions properly.
...
Northrays provides methods to handle errors when executing processes. You should handle process exceptions properly, log error details for debugging, and use try-catch blocks for error handling.

<Tabs syncKey="language">
<TabItem label="Python" icon="seti:python">
```python
from northrays import NorthraysError
...
<TabItem label="TypeScript" icon="seti:typescript">
```typescript
import { NorthraysError } from '@northrays/sdk'
...
try {
    ...
    if (e instanceof NorthraysError) {
...
Northrays provides solutions for troubleshooting common issues related to process and code execution.
