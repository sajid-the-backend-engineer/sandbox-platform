...
```python
def __init__(api_client: FileSystemApi, http_client: httpx.Client)
```
...
- `http_client` _httpx.Client_ - Shared pooled client for upload/download.
...
- `error_details` _FileDownloadErrorDetails | None_ - Structured error metadata when the server provides it.
