...
#### AsyncNorthrays.\_\_init\_\_

```python
def __init__(config: NorthraysConfig | None = None)
```
...
- `config` _NorthraysConfig | None_ - Object containing api_key, api_url, and target.
...
- `api_key` _str | None_ - API key for authentication with the Northrays API. If not set, it must be provided
  via the environment variable `NORTHRAYS_API_KEY`, or a JWT token must be provided instead.
- `jwt_token` _str | None_ - JWT token for authentication with the Northrays API. If not set, it must be provided
...
- `api_url` _str | None_ - URL of the Northrays API. Defaults to `'https://app.northrays.com/api'` if not set
