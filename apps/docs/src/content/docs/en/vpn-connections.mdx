...
description: Connect your sandboxes to private networks.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

VPN connections are a way to connect your Northrays sandboxes to private networks. By establishing a VPN connection, your sandbox can access network resources using private IP addresses and can be accessed by other devices on the same VPN network.
...
Northrays supports the following VPN network providers:
...
:::note
For connecting to a VPN network, you need to [create or access your Northrays sandbox](/docs/en/sandboxes), **have access to your VPN network provider credentials**, and be on [**Tier 3** or higher](/docs/en/limits#resources).
:::

## Tailscale

Northrays provides multiple ways to connect to a Northrays sandbox with a Tailscale network:
...
When you connect a Northrays sandbox to a Tailscale network, the sandbox becomes part of your private Tailscale network, allowing you to access resources that are available within the network and enabling other devices on the network to access the sandbox.
...
# Configuration
NORTHRAYS_API_KEY = "YOUR_API_KEY" # Replace with your API key

# Initialize the Northrays client
config = NorthraysConfig(api_key=NORTHRAYS_API_KEY)
northrays = Northrays(config)

def setup_tailscale_vpn_interactive():
    """
    Connect a Northrays sandbox to a Tailscale network using the Python SDK.
...
// Configuration
const NORTHRAYS_API_KEY = 'YOUR_API_KEY' // Replace with your API key

// Initialize the Northrays client
const northrays = new Northrays({
  apiKey: NORTHRAYS_API_KEY,
})
...
async function setupTailscaleVpnInteractive(): Promise<void> {
  /**
   * Connect a Northrays sandbox to a Tailscale network using the TypeScript SDK.
   * Uses interactive login via browser URL (no auth key required).
   */
...
Using an auth key provides a non-interactive way to connect your Northrays sandbox to Tailscale, making it suitable for automated scripts, CI/CD pipelines, or any scenario where manual browser interaction is not available.
...
Copy the auth key from the generated script and use it to connect your Northrays sandbox to Tailscale:
...
For working directly in the terminal or for more control over the Tailscale connection process, you can set up Tailscale manually through the Northrays [web terminal](/docs/en/web-terminal) or [SSH](/docs/en/ssh-access).
...
1. Navigate to your sandbox [web terminal](/docs/en/web-terminal) in [Northrays Dashboard ↗](https://app.northrays.com/), or [access it via SSH](/docs/en/ssh-access)
...
This begins the Tailscale installation process and initializes the Tailscale CLI inside the sandbox. Northrays requires the Tailscale daemon to be running in the background to connect the sandbox to the Tailscale network.
...
You've now successfully connected your Northrays sandbox to your Tailscale network. The sandbox should appear in your [Tailscale dashboard](https://login.tailscale.com/admin/machines).

## OpenVPN

Northrays provides multiple ways to connect to a Northrays sandbox with an OpenVPN network:

- [OpenVPN client configuration](#openvpn-client-configuration)
- [Connect with web terminal](#openvpn-web-terminal)

OpenVPN uses a client-server model where your Northrays sandbox acts as a client connecting to an OpenVPN server. This approach is suitable for connecting to existing corporate VPNs, accessing resources behind firewalls, or integrating with infrastructure that already uses OpenVPN.

:::note
Connecting a Northrays sandbox to OpenVPN network requires a [client configuration file](#openvpn-client-configuration-file).
:::
...
def setup_openvpn(ovpn_config: str):
    """
    Connect a Northrays sandbox to an OpenVPN network using the Python SDK.
    """
    # Create the sandbox
    print("Creating sandbox...")
    sandbox = northrays.create()
    ...
    sandbox.fs.upload_file(ovpn_config.encode(), "/home/northrays/client.ovpn")
    print("Configuration written to /home/northrays/client.ovpn")

    # Step 3: Start OpenVPN in background
    print("\nStarting OpenVPN tunnel...")
    sandbox.process.exec(
        "nohup sudo openvpn /home/northrays/client.ovpn > /tmp/openvpn.log 2>&1 &",
        timeout=10
    )
...
async function setupOpenvpn(ovpnConfig: string): Promise<void> {
  /**
   * Connect a Northrays sandbox to an OpenVPN network using the TypeScript SDK.
   */
  ...
  await sandbox.process.executeCommand(
    `cat << 'OVPNEOF' > /home/northrays/client.ovpn
${ovpnConfig}
OVPNEOF`,
    undefined,
    undefined,
  ...
  console.log("Configuration written to /home/northrays/client.ovpn");
...
Northrays provides a [web terminal](/docs/en/web-terminal) for interacting with your sandboxes, allowing you to install OpenVPN and connect to your OpenVPN network.

1. Navigate to your sandbox [web terminal](/docs/en/web-terminal) in [Northrays Dashboard ↗](https://app.northrays.com/) by clicking on the Terminal icon `>_`, or [access it via SSH](/docs/en/ssh-access)
...
3. Create the OpenVPN [client configuration file](#openvpn-client-configuration-file) for your Northrays sandbox
...
Northrays requires the OpenVPN tunnel to be running in the background to connect the sandbox to the OpenVPN network. The recommended approach is to run it in a detached tmux (or similar) session:
...
You've now successfully connected your Northrays sandbox to your OpenVPN network.

## Netbird

Northrays provides multiple ways to connect to a Northrays sandbox with a Netbird network:

- [Connect with setup key](#netbird-setup-key)
- [Connect with browser login](#netbird-browser-login)

When you connect a Northrays sandbox to a Netbird network, the sandbox becomes part of your private Netbird network, allowing you to access resources that are available within the network and enabling other devices on the network to access the sandbox.
...
Using a Netbird setup key provides a non-interactive way to connect your Northrays sandbox to Netbird, making it suitable for automated scripts, CI/CD pipelines, or any scenario where manual browser interaction is not available.
...
def setup_netbird():
    """
    Connect a Northrays sandbox to a NetBird network using the Python SDK.
...
// Configuration
const NORTHRAYS_API_KEY = "YOUR_API_KEY";
...
async function setupNetbird(): Promise<void> {
  /**
   * Connect a Northrays sandbox to a NetBird network using the TypeScript SDK.
   * Uses the official NetBird install script and a Setup Key for non-interactive auth.
   * See: https://docs.netbird.io/manage/peers/access-infrastructure/setup-keys-add-servers-to-network
   */
...
The browser login method initiates an interactive authentication flow where Netbird generates a unique login URL that you visit in your web browser to authenticate the Northrays sandbox.
