...
description: Mount external object storage (Amazon S3, Cloudflare R2, Google Cloud Storage, Azure Blob) and filesystems into Northrays sandboxes using FUSE.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

Mount object storage (Amazon S3, Cloudflare R2, Google Cloud Storage, Azure Blob) and filesystems like Archil and MesaFS into a Northrays sandbox as a regular directory. The sandbox reads from and writes to the bucket as if it were a local directory, so existing tools, scripts, and agents work without changes. This is useful for bringing in datasets, model weights, or build artifacts that already live in your own cloud account.

External storage mounts and [Northrays Volumes](/docs/en/volumes/) are complementary FUSE-based mechanisms — both expose remote object storage as a regular sandbox directory, both can be shared across sandboxes, and both persist beyond any individual sandbox's lifetime. The main distinction is **where the data physically lives**: Northrays Volumes are hosted on Northrays's own S3-compatible object store, while external mounts connect to a bucket or filesystem hosted on another provider (Amazon S3, Cloudflare R2, GCS, Azure Blob, Archil, MesaFS).

External storage is mounted using FUSE. Northrays supports two approaches, and each provider section below shows both — pick whichever fits your workflow:
...
```python
from northrays import CreateSnapshotParams, Northrays, Image

northrays = Northrays()

image = (
  Image.base("northrays/sandbox")
  .run_commands(
    "sudo apt-get update "
...
const image = Image.base('northrays/sandbox').runCommands(
  'sudo apt-get update ' +
    '&& sudo apt-get install -y --no-install-recommends libfuse2 ca-certificates wget',
  'arch="$(dpkg --print-architecture | sed s/amd64/x86_64/)" ' +
    '&& wget -O /tmp/mount-s3.deb ' +
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-s3', image: image),
...
image := northrays.Base("northrays/sandbox").
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            Image image = Image.base("northrays/sandbox")
                .runCommands(
                    "sudo apt-get update "
                        + "&& sudo apt-get install -y --no-install-recommends libfuse2 ca-certificates wget",
                    "arch=\"$(dpkg --print-architecture | sed s/amd64/x86_64/)\" "
            ...
            northrays.snapshot().create("fuse-s3", image, System.out::println);
        }
    }
}
```
...
sandbox.process.exec(f"echo 'hello from Northrays' > {mount_path}/hello.txt")
...
const mountPath = '/home/northrays/s3'
...
// Read and write through the mount as if it were a local directory
await sandbox.process.executeCommand(`echo 'hello from Northrays' > ${mountPath}/hello.txt`)
...
# Read and write through the mount as if it were a local directory
sandbox.process.exec(command: "echo 'hello from Northrays' > #{mount_path}/hello.txt")
...
// Read and write through the mount as if it were a local directory
if _, err := sandbox.Process.ExecuteCommand(ctx, "echo 'hello from Northrays' > "+mountPath+"/hello.txt"); err != nil {
	log.Fatal(err)
}
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            String mountPath = "/home/northrays/s3";
            ...
            sandbox.getProcess().executeCommand(
                "echo 'hello from Northrays' > " + mountPath + "/hello.txt");
...
```python
import os
from northrays import CreateSandboxBaseParams, Northrays
...
sandbox = northrays.create(
  Northrays::CreateSandboxBaseParams.new(
    env_vars: {
      'AWS_ACCESS_KEY_ID' => ENV.fetch('AWS_ACCESS_KEY_ID'),
      'AWS_SECRET_ACCESS_KEY' => ENV.fetch('AWS_SECRET_ACCESS_KEY')
    }
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-r2', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            northrays.snapshot().create("fuse-r2", image, System.out::println);
        }
    }
}
```
...
const mountPath = '/home/northrays/r2'
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            String mountPath = "/home/northrays/r2";
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-tigris', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            northrays.snapshot().create("fuse-tigris", image, System.out::println);
        }
    }
}
```
...
mount_path = "/home/northrays/tigris"
...
const mountPath = '/home/northrays/tigris'
...
mountPath := "/home/northrays/tigris"
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            String mountPath = "/home/northrays/tigris";
...
import io.northrays.sdk.Northrays;
import io.northrays.sdk.Sandbox;
import io.northrays.sdk.model.CreateSandboxBaseParams;
import io.northrays.sdk.model.ExecuteResponse;
...
This pattern fits Northrays sandboxes when each sandbox needs a writable copy of a shared dataset (model weights, fixtures, golden data) without duplicating it on every launch.
...
:::note
Northrays's base image is Debian Trixie, but Google has not published a Trixie-specific gcsfuse repository yet. The `gcsfuse-bookworm` repo packages run cleanly on Trixie, so we use them in the snippets below.
:::
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-gcs', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            northrays.snapshot().create("fuse-gcs", image, System.out::println);
        }
    }
}
```
...
sandbox = northrays.create(CreateSandboxFromSnapshotParams(snapshot="fuse-gcs"))

mount_path = "/home/northrays/gcs"
key_path = "/home/northrays/.gcs-key.json"
...
const sandbox = await northrays.create({ snapshot: 'fuse-gcs' })

const mountPath = '/home/northrays/gcs'
const keyPath = '/home/northrays/.gcs-key.json'
...
sandbox = northrays.create(
  Northrays::CreateSandboxFromSnapshotParams.new(snapshot: 'fuse-gcs')
)
...
mountPath := "/home/northrays/gcs"
keyPath := "/home/northrays/.gcs-key.json"
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            String mountPath = "/home/northrays/gcs";
            String keyPath = "/home/northrays/.gcs-key.json";
...
sandbox = northrays.create(CreateSandboxBaseParams())
...
sandbox = northrays.create(Northrays::CreateSandboxBaseParams.new)
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            Sandbox sandbox = northrays.create(new CreateSandboxFromSnapshotParams());
...
:::caution
Northrays's base image is Debian Trixie. Microsoft's `blobfuse2` Bookworm binary links against `libfuse3.so.3`, but Trixie's `fuse3` package bumped the SONAME to `.4`, so `libfuse3.so.3` is missing on disk. The snapshot recipe below creates a compatibility symlink. Without it, `blobfuse2` fails with `libfuse3.so.3: cannot open shared object file`.
:::
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-azure', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            northrays.snapshot().create("fuse-azure", image, System.out::println);
        }
    }
}
```
...
sandbox = northrays.create(CreateSandboxFromSnapshotParams(snapshot="fuse-azure"))

mount_path = "/home/northrays/azure"
config_path = "/home/northrays/.blobfuse2.yaml"
...
const sandbox = await northrays.create({ snapshot: 'fuse-azure' })

const mountPath = '/home/northrays/azure'
const configPath = '/home/northrays/.blobfuse2.yaml'
...
sandbox = northrays.create(
  Northrays::CreateSandboxFromSnapshotParams.new(snapshot: 'fuse-azure')
)
...
mountPath := "/home/northrays/azure"
configPath := "/home/northrays/.blobfuse2.yaml"
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            String mountPath = "/home/northrays/azure";
            String configPath = "/home/northrays/.blobfuse2.yaml";
...
Create a reusable snapshot that installs the `archil` CLI. The `northrays/sandbox` base image ships Debian Trixie which doesn't include `libfuse2`, so we apt-install it first; the Archil installer reads the `.deb` it downloads and links against `libfuse2`.
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-archil', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            northrays.snapshot().create("fuse-archil", image, System.out::println);
        }
    }
}
```
...
Pass `ARCHIL_MOUNT_TOKEN`, `ARCHIL_REGION`, and `ARCHIL_DISK` to the sandbox via `envVars`. Your code then mounts the disk at `/home/northrays/archil` and hands ownership to the `northrays` user so non-root processes can read and write through the mount.
...
mount_path = "/home/northrays/archil"
...
sandbox.process.exec(f"sudo chown northrays:northrays {mount_path}")
...
const mountPath = '/home/northrays/archil'
...
await sandbox.process.executeCommand(`sudo chown northrays:northrays ${mountPath}`)
...
sandbox.process.exec(command: "sudo chown northrays:northrays #{mount_path}")
...
mountPath := "/home/northrays/archil"
...
if _, err := sandbox.Process.ExecuteCommand(ctx,
	"sudo chown northrays:northrays "+mountPath); err != nil {
	log.Fatal(err)
}
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            String mountPath = "/home/northrays/archil";
            ...
            sandbox.getProcess().executeCommand(
                "sudo --preserve-env=ARCHIL_MOUNT_TOKEN archil mount "
                + "$ARCHIL_DISK " + mountPath + " --region $ARCHIL_REGION");
            sandbox.getProcess().executeCommand("sudo chown northrays:northrays " + mountPath);
...
To let multiple sandboxes mount the **same** disk concurrently, add `--shared` to `archil mount`. In shared mode you check out paths with `archil checkout <path>` before writing, and check them back in with `archil checkin <path>` when done. See [Shared Disks ↗](https://docs.archil.com/concepts/shared-disks).
...
[MesaFS ↗](https://mesa.dev) is an agent-native versioned filesystem from Mesa, purpose-built for the same workloads Northrays sandboxes run — parallel agent swarms, shared working memory, structured artifacts, and long-lived state across runs. With MesaFS, instead of mounting a cloud bucket, you mount a Mesa **repository**: a Git-compatible versioned working directory with sub-50ms reads/writes, instant fork/branch operations, and unlimited concurrent writers.

The Mesa setup follows the same pattern as the bucket providers but uses the Mesa CLI rather than a FUSE-specific tool: install the CLI in your sandbox, authenticate with your API key, and run `mesa mount --daemonize` to mount your repos at `/home/northrays/mesa/mnt/<org>/<repo>`.
...
northrays.snapshot.create(
  Northrays::CreateSnapshotParams.new(name: 'fuse-mesa', image: image),
  on_logs: proc { |chunk| print(chunk) }
)
```
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            northrays.snapshot().create("fuse-mesa", image, System.out::println);
        }
    }
}
```
...
Pass `MESA_API_KEY` and your Mesa organization slug to the sandbox via `envVars`. Your code then writes a TOML config into the sandbox, authenticates the Mesa CLI, and mounts your repos at `/home/northrays/mesa/mnt/<org>/<repo>`.
...
repo = "my-workspace"
mount_path = f"/home/northrays/mesa/mnt/{org}/{repo}"
config_path = "/home/northrays/.config/mesa/config.toml"
...
config = f'''mount-point = "/home/northrays/mesa/mnt"
...
const mountPath = `/home/northrays/mesa/mnt/${org}/${repo}`
const configPath = '/home/northrays/.config/mesa/config.toml'
...
const config = `mount-point = "/home/northrays/mesa/mnt"
...
repo = 'my-workspace'
mount_path = "/home/northrays/mesa/mnt/#{org}/#{repo}"
config_path = '/home/northrays/.config/mesa/config.toml'
...
config = <<~TOML
  mount-point = "/home/northrays/mesa/mnt"
...
repo := "my-workspace"
mountPath := fmt.Sprintf("/home/northrays/mesa/mnt/%s/%s", org, repo)
configPath := "/home/northrays/.config/mesa/config.toml"
...
config := fmt.Sprintf(`mount-point = "/home/northrays/mesa/mnt"
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            ...
            String mountPath = "/home/northrays/mesa/mnt/" + org + "/" + repo;
            String configPath = "/home/northrays/.config/mesa/config.toml";
            ...
            String config = "mount-point = \"/home/northrays/mesa/mnt\"\n\n"
                + "[secrets]\n"
                + "backend = \"plaintext-file\"\n\n"
                + "[organizations." + org + "]\n";
...
For repo-scoped or path-scoped keys, see Mesa's [auth and permissions guide ↗](https://docs.mesa.dev/content/getting-started/auth-and-permissions). For the full integration recipe, see Mesa's [Northrays guide ↗](https://docs.mesa.dev/content/integration-guides/northrays).

## Unmount

When a sandbox is deleted via `northrays.delete(sandbox)`, the container teardown automatically removes any active FUSE mounts and shuts down their daemons. For normal cleanup, this is all you need — no manual unmount required.
