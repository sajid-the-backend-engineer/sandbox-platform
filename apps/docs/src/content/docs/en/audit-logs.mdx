---
title: Audit Logs
description: View and monitor all user actions across your Northrays organization.
---

Northrays audit logs provide a detailed record of user and system activity across your organization. Use this feature to track sandbox lifecycle events, user access, system changes, and more.
...
Audit logs are available to [administrators](/docs/en/organizations#roles) with full access and [members](/docs/en/organizations#roles) with audit log permissions. Contact your organization administrator to get access to audit logs.

## Access from Dashboard

Access the audit logs page directly from [Northrays Dashboard ↗](https://app.northrays.com/dashboard/audit-logs). The audit logs page displays a list of all audit logs for your organization, including the following columns:
...
Northrays provides real-time updates of audit logs. Enable the **Auto Refresh** toggle in the top-right corner of the [Northrays Audit Logs ↗](https://app.northrays.com/dashboard/audit-logs) page to automatically refresh logs as new events occur.

## Programmatic management

Northrays provides API endpoints for programmatic access to audit logs.
...
```bash
curl https://app.northrays.com/api/audit \
...
```bash
curl https://app.northrays.com/api/audit/organizations/{organizationId} \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Below is the complete list of actions logged by Northrays:
