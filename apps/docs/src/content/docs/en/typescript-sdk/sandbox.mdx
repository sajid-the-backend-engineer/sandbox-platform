...
- `snapshot?` _string_ - Northrays snapshot used to create the Sandbox
...
- `NorthraysError` - If Sandbox fails to recover or times out
...
- `NorthraysError` - If interval is not a non-negative integer
...
- `NorthraysError` - If Sandbox fails to start or times out
...
- `NorthraysError` - If the sandbox ends up in an error state or fails to start within the timeout period.
...
- `NorthraysError` - If the sandbox fails to stop within the timeout period.
## ListSandboxesQuery
