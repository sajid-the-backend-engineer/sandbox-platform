...
- `snapshot` _SnapshotService_ - Service for managing Northrays Snapshots
- `volume` _VolumeService_ - Service for managing Northrays Volumes
...
```ts
new Northrays(config?: NorthraysConfig): Northrays
```
...
- `config?` _NorthraysConfig_ - Configuration options
...
- `apiKey?` _string_ - API key for authentication with the Northrays API
- `apiUrl?` _string_ - URL of the Northrays API. Defaults to 'https://app.northrays.com/api'
    if not set here and not set in environment variable NORTHRAYS_API_URL.
- `jwtToken?` _string_ - JWT token for authentication with the Northrays API. If not set, it must be provided
