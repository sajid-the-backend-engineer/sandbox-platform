...
description: Interact with Git repositories in your sandboxes.
---

import { TabItem, Tabs } from '@astrojs/starlight/components'

Northrays provides built-in Git support through the `git` module in sandboxes.

## Basic operations

Northrays provides methods to clone, check status, and manage Git repositories in sandboxes.
...
Northrays provides methods to clone Git repositories into sandboxes. You can clone public or private repositories, specific branches, and authenticate using personal access tokens.
...
Northrays provides methods to check the status of Git repositories in sandboxes. You can get the current branch, modified files, number of commits ahead and behind main branch.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/status?path='
```
...
Northrays provides methods to manage branches in Git repositories. You can create, switch, and delete branches.

### Create branches

Northrays provides methods to create branches in Git repositories. The following snippet creates a new branch called `new-feature`.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/branches' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "name": "",
...
Northrays provides methods to checkout branches in Git repositories. The following snippet checks out the branch called `feature-branch`.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/checkout' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "branch": "",
...
Northrays provides methods to delete branches in Git repositories. The following snippet deletes the branch called `old-feature`.
...
Northrays provides methods to stage changes in Git repositories. You can stage specific files, all changes, and commit with a message. The following snippet stages the file `file.txt` and the `src` directory.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/add' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "files": [
...
Northrays provides methods to commit changes in Git repositories. You can commit with a message, author, and email. The following snippet commits the changes with the message `Update documentation` and the author `John Doe` and email `john@example.com`.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/commit' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "allow_empty": true,
...
Northrays provides methods to work with remote repositories in Git. You can push and pull changes from remote repositories.

### Push changes

Northrays provides methods to push changes to remote repositories. The following snippet pushes the changes to a public repository.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/push' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "password": "",
...
Northrays provides methods to pull changes from remote repositories. The following snippet pulls the changes from a public repository.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/git/pull' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "password": "",
