...
Northrays provides Language Server Protocol (LSP) support through sandbox instances. This enables advanced language features like code completion, diagnostics, and more.

## Create LSP servers

Northrays provides methods to create LSP servers. The `path_to_project` argument is relative to the current sandbox working directory when no leading `/` is used. The working directory is specified by WORKDIR when it is present in the Dockerfile, and otherwise falls back to the user's home directory.
...
The supported languages for creating LSP servers with Northrays are defined by the `LspLanguageId` enum:
...
Northrays provides methods to start LSP servers.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/start' \
  --request POST \
...
Northrays provides methods to stop LSP servers.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/stop' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "languageId": "",
...
Northrays provides methods to get code completions for a specific position in a file.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/completions' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "context": {
...
Northrays provides methods to notify the LSP server when files are opened or closed. This enables features like diagnostics and completion tracking for the specified files.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/did-open' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "languageId": "",
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/did-close' \
  --request POST \
  --header 'Content-Type: application/json' \
  --data '{
  "languageId": "",
...
Northrays provides methods to retrieve symbols (functions, classes, variables, etc.) from a document.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/document-symbols?languageId=&pathToProject=&uri='
```
...
Northrays provides methods to search for symbols across all files in the sandbox.
...
```bash
curl 'https://proxy.app.northrays.com/toolbox/{sandboxId}/lsp/workspacesymbols?query=&languageId=&pathToProject='
```
