...
Sandbox class for Northrays SDK.
...
- `NorthraysApiClient:SandboxState` - The state of the sandbox
...
- `NorthraysApiClient:SandboxDesiredState` - The desired state of the sandbox
...
- `Array\<NorthraysApiClient:SandboxVolume\>, nil` - Volumes attached to the sandbox.
Not returned by list results; call #refresh on each item to populate.
...
- `NorthraysApiClient:BuildInfo, nil` - Build information for the sandbox if it was
...
- `NorthraysApiClient:SshAccessDto`
...
- `NorthraysApiClient:PortPreviewUrl`
...
- `NorthraysApiClient:SignedPortPreviewUrl` - The signed preview URL response object
...
- `resources` _Northrays:Resources_ - New resource configuration (cpu, memory, disk only)
- `timeout` _Numeric_ - Maximum wait time in seconds (defaults to 60 s)
...
- `language_id` _Symbol_ - The language server type (e.g., Northrays::LspServer::Language::PYTHON)
...
- `NorthraysApiClient:SshAccessValidationDto`
...
- `Northrays:Sandbox` - The forked Sandbox
