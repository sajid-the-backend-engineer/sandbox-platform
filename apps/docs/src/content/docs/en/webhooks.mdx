---
title: Webhooks
description: Connect your applications to Northrays events in real-time with webhooks for automation, monitoring, and integrations.
---

Webhooks are HTTP callbacks that Northrays sends to your specified endpoints when specific events occur.
Think of them as "reverse API calls" - instead of your application asking Northrays for updates, Northrays
...
- **Integration**: connect Northrays with existing tools like Slack, Discord, or custom applications
- **Audit & compliance**: maintain detailed logs of all important changes

:::note
Webhooks are available to organization admins and members with appropriate permissions. If you don't see **Webhooks** in [Northrays Dashboard ↗](https://app.northrays.com/dashboard), contact [support@northrays.com](mailto:support@northrays.com) to enable webhooks for your organization.
:::

## Accessing webhooks

Northrays provides a webhook management interface to access and manage webhook endpoints.
...
Northrays provides a webhook management interface to create webhook endpoints.
...
Northrays provides a webhook management interface to edit webhook endpoints.
...
Northrays provides a webhook management interface to delete webhook endpoints.
...
Northrays sends webhooks for lifecycle events across your infrastructure resources. You can subscribe to specific event types or receive all events and filter them in your application.
