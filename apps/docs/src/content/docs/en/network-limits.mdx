...
Northrays provides network egress limiting for sandboxes to control internet access. This feature can be automatically applied based on your [organization's limits](/docs/en/limits/) or manually configured for specific sandboxes.
...
Northrays provides methods to control network access when [creating sandboxes](/docs/en/sandboxes#create-sandboxes) by using the `networkAllowList` and `networkBlockAll` parameters:
...
```bash
# Allow access to specific IP addresses (Wikipedia, X/Twitter, private network)
northrays create --network-allow-list '208.80.154.232/32,199.16.156.103/32,192.168.1.0/24'

# Or block all network access
northrays create --network-block-all
```
...
```bash
curl 'https://app.northrays.com/api/sandbox/SANDBOX_ID_OR_NAME/network-settings' \
  --request POST \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
...
Northrays provides a list of essential services that are available on all tiers and can be used for development.

:::note
This list is continuously updated. If you require access to additional essential development services, submit a request in the [sandbox network whitelist](https://github.com/northrays/sandbox-network-whitelist) repository or contact [support@northrays.com](mailto:support@northrays.com).
:::
...
1. Verify your [organization tier](/docs/en/limits#tiers) in the [Northrays Dashboard ↗](https://app.northrays.com/dashboard/limits)
2. Verify your [network allow list](#network-allow-list-format) configuration
3. Contact [support@northrays.com](mailto:support@northrays.com) for assistance
