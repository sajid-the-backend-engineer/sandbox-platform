...
- **Sandbox SDKs**: [TypeScript](/docs/en/typescript-sdk/sandbox), [Python](/docs/en/python-sdk/sync/sandbox), [Ruby](/docs/en/ruby-sdk/sandbox), [Go](/docs/en/go-sdk/northrays#type-sandbox), [Java](/docs/en/java-sdk/sandbox)
- **Sandbox API**: [RESTful API](/docs/en/tools/api/#northrays/tag/sandbox) ([OpenAPI spec](/docs/openapi.json)), [Toolbox API](/docs/en/tools/api#northrays-toolbox) ([OpenAPI spec](/docs/toolbox-openapi.json))
- **Sandbox CLI**: [Mac/Linux/Windows](/docs/en/tools/cli)

## Create Sandboxes

Northrays provides methods to create sandboxes.

1. Navigate to [Northrays Sandboxes ↗](https://app.northrays.com/dashboard/sandboxes)
...
Northrays provides methods to create GPU sandboxes.

Northrays supports NVIDIA GPU devices for creating GPU sandboxes. Use GPU sandboxes for workloads such as model inference, fine-tuning, and CUDA-accelerated compute.

- **NVIDIA H100**
- **NVIDIA RTX Pro 6000**

Northrays provides a pre-built `northrays-gpu` snapshot for creating GPU sandboxes. Each GPU sandbox is ephemeral and supports up to **16 vCPUs**, **192GB RAM**, and **512GB disk**.

1. Navigate to [Northrays Sandboxes ↗](https://app.northrays.com/dashboard/sandboxes)
2. Click **Create Sandbox**
3. Select a GPU snapshot (**`northrays-gpu`**)
...
```python
from northrays import Northrays, NorthraysConfig, CreateSandboxFromSnapshotParams

northrays = Northrays(NorthraysConfig(target="us-east-1"))
sandbox = northrays.create(
        CreateSandboxFromSnapshotParams(
            snapshot="northrays-gpu",
...
public class App {
    public static void main(String[] args) {
        ...
        try (Northrays northrays = new Northrays(config)) {
            ...
            params.setSnapshot("northrays-gpu");
...
```bash
northrays create --snapshot northrays-gpu --target us-east-1 --auto-delete 0
```
...
```python
from northrays import Northrays, CreateSandboxFromImageParams, Image, Resources, GpuType, NorthraysConfig
...
```typescript
import { Northrays, GpuType, Image } from "@northrays/sdk";
...
sandbox = northrays.create(
  Northrays::CreateSandboxFromImageParams.new(
    ...
    resources: Northrays::Resources.new(
      gpu: 1,
      gpu_type: [Northrays::GpuType::H100, Northrays::GpuType::RTX_PRO_6000]
    )
  )
)
```
...
Northrays provides methods to create Windows sandboxes.
...
Northrays provides a pre-built `windows` snapshot for creating Windows sandboxes. The snapshot uses **2 vCPU**, **8GiB** memory, and **30GiB** disk.
...
```bash
northrays create --snapshot windows --target us
```
...
Northrays provides methods to create linked sandboxes. 
...
parent = northrays.create()

child = northrays.create(
    CreateSandboxFromSnapshotParams(
...
const parent = await northrays.create();

const child = await northrays.create({
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            Sandbox parent = northrays.create();
            ...
            Sandbox child = northrays.create(childParams);
...
Northrays provides methods to create ephemeral sandboxes.
...
northrays create --auto-delete 0 --auto-stop 5
```
...
Northrays provides [pre-built snapshots](/docs/en/snapshots#default-snapshots) with fixed resource sizes for creating sandboxes.
...
public class App {
    public static void main(String[] args) {
        try (Northrays northrays = new Northrays()) {
            CreateSandboxFromSnapshotParams params = new CreateSandboxFromSnapshotParams();
            params.setSnapshot("northrays-medium");
            Sandbox sandbox = northrays.create(params);
        }
    }
}
...
```bash
northrays create --snapshot northrays-medium
```
...
Northrays provides methods to create sandboxes with custom resources.

Use the `Resources` class to set custom sandbox resources. All resource parameters are optional and must be integers. If not specified, Northrays will use the default values. Maximum values are per-sandbox limits set at the organization level.
...
```bash
northrays create --cpu 2 --memory 4 --disk 8
```
...
Northrays provides methods to start sandboxes.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/start' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to get a sandbox by ID or name.
...
```java
Sandbox sandbox = northrays.get("my-sandbox-id-or-name");
```
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}' \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to list sandboxes.
...
```ruby
northrays.list.each { |sandbox| puts sandbox.id }
```
...
```java
Iterator<Map<String, Object>> iter = northrays.list();
while (iter.hasNext()) {
...
Northrays provides methods to stop sandboxes.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/stop' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to pause sandboxes.
...
Northrays supports pause functionality through VM-based runners. Pause is handled through the existing stop action. This means stop behaves as pause and preserves memory state, while force stop performs a full shutdown without preserving memory state.

## Archive Sandboxes

Northrays provides methods to archive sandboxes.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/archive' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to recover sandboxes.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/recover' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to resize [sandbox resources](#resources) after creation.
...
```ruby
# Resize a started sandbox (CPU and memory can be increased)
sandbox.resize(Northrays::Resources.new(cpu: 2, memory: 4))

# Resize a stopped sandbox (CPU and memory can change, disk can only increase)
sandbox.stop
sandbox.resize(Northrays::Resources.new(cpu: 4, memory: 8, disk: 20))
sandbox.start
```
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/resize' \
  --request POST \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
  --data '{
...
Northrays provides methods to fork sandboxes.
...
Northrays tracks the parent-child relationship in a fork tree, so you can always trace a fork's lineage back to the sandbox it was created from. You can fork a fork, building out branches as needed. The parent sandbox cannot be deleted while it has active fork children.
...
// Or use the Northrays helper method
const forkedSandbox = await northrays._experimental_fork(sandbox, { name: "my-forked-sandbox" });
```
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/fork' \
  --request POST \
  --header 'X-Northrays-Organization-ID: YOUR_ORGANIZATION_ID' \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
  --data '{
  "name": "my-forked-sandbox"
...
Northrays provides methods to set sandbox labels.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/labels' \
  --request PUT \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
  --data '{
...
Northrays provides methods to create [snapshots](/docs/en/snapshots) from sandboxes.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/snapshot' \
  --request POST \
  --header 'X-Northrays-Organization-ID: YOUR_ORGANIZATION_ID' \
  --header 'Content-Type: application/json' \
  --header 'Authorization: Bearer YOUR_API_KEY' \
...
Northrays provides methods to delete sandboxes.
...
Northrays sandboxes support Python, TypeScript, and JavaScript programming language runtimes for direct code execution inside the sandbox. The `language` parameter controls which programming language runtime is used for the sandbox:

- **`python`**
- **`typescript`**
- **`javascript`**

If omitted, the Northrays SDK will default to `python`. To override this, explicitly set the `language` value when creating the sandbox.
...
Northrays provides methods to update a sandbox's last activity timestamp.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxId}/last-activity' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to set the auto-stop interval.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/autostop/{interval}' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
- API requests to the [Northrays Toolbox SDK](/docs/en/tools/api/#northrays-toolbox)
...
Northrays provides methods to set the auto-archive interval.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/autoarchive/{interval}' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to set the auto-delete interval.
...
```bash
curl 'https://app.northrays.com/api/sandbox/{sandboxIdOrName}/autodelete/{interval}' \
  --request POST \
  --header 'Authorization: Bearer YOUR_API_KEY'
```
...
Northrays provides methods to run sandboxes indefinitely.

By default, Northrays sandboxes auto-stop after 15 minutes of inactivity. To keep a sandbox running without interruption, set the auto-stop interval to `0` when creating a new sandbox:
