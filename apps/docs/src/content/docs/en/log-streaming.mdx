---
title: Log Streaming
description: Stream logs as they are being produced, while the process is still running.
---
...
```typescript
import { Northrays, SessionExecuteRequest } from '@northrays/sdk'
...
command = sandbox.process.execute_session_command(
  session_id,
  Northrays::SessionExecuteRequest.new(
    command: 'for i in {1..5}; do echo "Step $i"; echo "Error $i" >&2; sleep 1; done',
    var_async: true
  )
)
