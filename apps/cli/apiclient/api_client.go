...
import (
	...
	"github.com/northrays/sandbox-platform/cli/auth"
...
const API_VERSION_HEADER = "X-Northrays-Api-Version"
...
func checkVersionsMismatch(res *http.Response) {
	...
	versionMismatchWarningOnce.Do(func() {
		log.Warn(fmt.Sprintf("Version mismatch: Northrays CLI is on v%s and API is on v%s.\nMake sure the versions are aligned using 'brew upgrade northrays/cli/northrays' or by downloading the latest version from https://github.com/northrays/sandbox-platform/releases.", cliVersion, apiVersion))
	})
}
...
func GetApiClient(profile *config.Profile, defaultHeaders map[string]string) (*apiclient.APIClient, error) {
	...
	if activeProfile.Api.Key != nil {
		...
		if activeProfile.ActiveOrganizationId != nil {
			clientConfig.AddDefaultHeader("X-Northrays-Organization-ID", *activeProfile.ActiveOrganizationId)
		}
	}

	clientConfig.AddDefaultHeader(NorthraysSourceHeader, "cli")
