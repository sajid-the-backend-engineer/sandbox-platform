...
var LoginCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		items := []view_common.SelectItem{
			...
			{Title: "Set Northrays API Key", Desc: "Authenticate using Northrays API key"},
		}
		...
		setApiKey := choice == "Set Northrays API Key"

		if setApiKey {
			// Prompt for API key
			apiKey, err := view_common.PromptForInput("", "Enter your Northrays API key", "You can find it in the Northrays dashboard - https://app.northrays.com/dashboard")
			if err != nil {
				return err
			}
...
func updateProfileWithLogin(tokenConfig *config.Token, apiKey *string) error {
	...
	if apiKey != nil {
		...
		view_common.RenderInfoMessageBold("Successfully set Northrays API key!")
	}
