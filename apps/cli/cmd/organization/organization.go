...
var OrganizationCmd = &cobra.Command{
	Use:     "organization",
	Short:   "Manage Northrays organizations",
	Long:    "Commands for managing Northrays organizations",
	Aliases: []string{"organizations", "org", "orgs"},
	GroupID: internal.USER_GROUP,
	PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
		if config.IsApiKeyAuth() {
			return errors.New("organization commands are not available when using API key authentication - run `northrays login` to reauthenticate with browser")
		}
