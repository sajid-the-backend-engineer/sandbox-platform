...
export const PythonSnippetGenerator: CodeSnippetGenerator = {
  ...
  getConfig(p) {
    ...
    return ['\n# Define the configuration', 'config = NorthraysConfig()'].filter(Boolean).join('\n') + '\n'
  },

  getClientInit(p) {
    return ['# Initialize the Northrays client', `northrays = Northrays(${p.actions.useConfigObject ? 'config' : ''})`]
      .filter(Boolean)
      .join('\n')
  },
  ...
  getSandboxCreate(p) {
    return [
      ...
      `sandbox = northrays.create(${p.config.useSandboxCreateParams ? 'params' : ''})`,
