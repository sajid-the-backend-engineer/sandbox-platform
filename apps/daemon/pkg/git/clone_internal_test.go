...
func TestBuildCloneArgs(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			repo: &gitprovider.GitRepository{
				Url:    "https://github.com/northrays/sandbox-platform",
				Branch: "main",
			},
			workDir: "/work-dir",
			expected: []string{
				...
				"--branch", "main",
				"--", "https://github.com/northrays/sandbox-platform", "/work-dir",
			},
		},
		{
			name: "http URL with branch",
			repo: &gitprovider.GitRepository{
				Url:    "http://localhost:3000/northrays/northrays",
				Branch: "main",
			},
			workDir: "/work-dir",
			expected: []string{
				...
				"clone", "--single-branch", "--progress",
				"--branch", "main",
				"--", "http://localhost:3000/northrays/northrays", "/work-dir",
			},
		},
		{
			name: "URL without protocol gets https:// prefix",
			repo: &gitprovider.GitRepository{
				Url:    "github.com/northrays/sandbox-platform",
				Branch: "main",
			},
			workDir: "/work-dir",
			expected: []string{
...
func TestBuildCloneEnv_OverridesBaseEnv(t *testing.T) {
	...
	require.Contains(t, env, "HOME=/home/northrays")
...
func TestBuildCloneEnv_PreservesBaseEnv(t *testing.T) {
	base := []string{"PATH=/usr/bin", "HOME=/home/northrays"}
