...
// Package northrays provides a Go SDK for interacting with the Northrays platform.
//
// The Northrays SDK enables developers to programmatically create, manage, and interact
...
//   - NORTHRAYS_API_URL: API URL (defaults to https://app.northrays.com/api)
//   - NORTHRAYS_TARGET: Target environment
//
// Or provide configuration explicitly:
//
//	client, err := northrays.NewClientWithConfig(&types.NorthraysConfig{
//	    APIKey: "your-api-key",
//	    APIUrl: "https://your-instance.northrays.com/api",
//	})
//
// # Creating Sandboxes
//
...
const (
	// defaultAPIURL is the default Northrays API endpoint used when no custom URL is configured.
	defaultAPIURL = "https://app.northrays.com/api"
...
// Client is the main entry point for interacting with the Northrays platform.
//
...
type Client struct {
	...
	apiURL          string             // Base URL for the Northrays API
...
// NewClient creates a new Northrays client with default configuration.
//
// NewClient reads configuration from environment variables:
//   - NORTHRAYS_API_KEY or NORTHRAYS_JWT_TOKEN for authentication (one is required)
//   - NORTHRAYS_ORGANIZATION_ID (required when using JWT token)
//   - NORTHRAYS_API_URL for custom API endpoint
//   - NORTHRAYS_TARGET for target environment
//
...
// NewClientWithConfig creates a new Northrays client with a custom configuration.
//
...
//	client, err := northrays.NewClientWithConfig(&types.NorthraysConfig{
//	    APIKey:         "your-api-key",
//	    APIUrl:         "https://custom.northrays.com/api",
...
func NewClientWithConfig(config *types.NorthraysConfig) (*Client, error) {
	client := &Client{
	...
	if client.apiKey == "" {
		client.apiKey = os.Getenv("NORTHRAYS_API_KEY")
	}

	if client.jwtToken == "" {
		client.jwtToken = os.Getenv("NORTHRAYS_JWT_TOKEN")
	}

	// Validate authentication
	if client.apiKey == "" && client.jwtToken == "" {
		return nil, errors.NewNorthraysError("Authentication credentials not found. Set NORTHRAYS_API_KEY, or both NORTHRAYS_JWT_TOKEN and NORTHRAYS_ORGANIZATION_ID. These can also be provided via NorthraysConfig.", 0, nil)
	}

	if client.organizationID == "" {
		client.organizationID = os.Getenv("NORTHRAYS_ORGANIZATION_ID")
	}

	if client.jwtToken != "" && client.organizationID == "" {
		return nil, errors.NewNorthraysError("NORTHRAYS_ORGANIZATION_ID is required when authenticating with NORTHRAYS_JWT_TOKEN. It can also be provided via NorthraysConfig.", 0, nil)
	}

	if client.apiURL == "" {
		client.apiURL = defaultAPIURL
		if apiURL := os.Getenv("NORTHRAYS_API_URL"); apiURL != "" {
			client.apiURL = apiURL
		} else if serverURL := os.Getenv("NORTHRAYS_SERVER_URL"); serverURL != "" {
			client.apiURL = serverURL
		}
	}

	if client.region == "" {
		client.region = os.Getenv("NORTHRAYS_TARGET")
	}
	...
	apiCfg.AddDefaultHeader("X-Northrays-Source", sdkSource)
	apiCfg.AddDefaultHeader("X-Northrays-SDK-Version", Version)
	...
	if client.jwtToken != "" {
		apiCfg.AddDefaultHeader("X-Northrays-Organization-ID", client.organizationID)
	}
	...
	otelEnabled := (config != nil && config.OtelEnabled) ||
		...
		os.Getenv("NORTHRAYS_OTEL_ENABLED") == "true" ||
		os.Getenv("NORTHRAYS_EXPERIMENTAL_OTEL_ENABLED") == "true"
	if otelEnabled {
		otelState, err := initOtel(context.Background())
		if err != nil {
			return nil, errors.NewNorthraysError(fmt.Sprintf("failed to initialize OpenTelemetry: %v", err), 0, nil)
		}
...
// handleAPIError converts API client errors to Northrays error types
func (c *Client) handleAPIError(err error, httpResp *http.Response) error {
	if httpResp == nil {
		return errors.NewNorthraysError(err.Error(), 0, nil)
	}
	...
	case http.StatusNotFound:
		return errors.NewNorthraysNotFoundError(message, httpResp.Header)
	case http.StatusTooManyRequests:
		return errors.NewNorthraysRateLimitError(message, httpResp.Header)
	default:
		return errors.NewNorthraysError(message, httpResp.StatusCode, httpResp.Header)
	}
}
...
func (c *Client) createToolboxClient(proxyURL string, sandboxID string) (*toolbox.APIClient, error) {
	...
	cfg.AddDefaultHeader("X-Northrays-Source", sdkSource)
	cfg.AddDefaultHeader("X-Northrays-SDK-Version", Version)
	cfg.UserAgent = "sdk-go/" + Version

	if c.jwtToken != "" {
		cfg.AddDefaultHeader("X-Northrays-Organization-ID", c.organizationID)
	}
...
func (c *Client) doCreate(ctx context.Context, params any, opts ...func(*options.CreateSandbox)) (*Sandbox, error) {
	...
	default:
		return nil, errors.NewNorthraysError(fmt.Sprintf("invalid %s: %s. Supported languages: python, javascript, typescript", types.CodeToolboxLanguageLabel, baseParams.Language), 0, nil)
	}
	...
	if baseParams.AutoStopInterval != nil && *baseParams.AutoStopInterval < 0 {
		return nil, errors.NewNorthraysError("autoStopInterval must be a non-negative integer", 0, nil)
	}
	if baseParams.AutoArchiveInterval != nil && *baseParams.AutoArchiveInterval < 0 {
		return nil, errors.NewNorthraysError("autoArchiveInterval must be a non-negative integer", 0, nil)
	}
	...
	if sandboxResp.GetState() == apiclient.SANDBOXSTATE_ERROR || sandboxResp.GetState() == apiclient.SANDBOXSTATE_BUILD_FAILED {
		return nil, errors.NewNorthraysError("Sandbox failed to start", 0, nil)
	}
...
// a [errors.NorthraysNotFoundError] is returned.
//
// Example:
//
//	sandbox, err := client.Get(ctx, "my-sandbox")
//	if err != nil {
//	    var notFound *errors.NorthraysNotFoundError
...
func (c *Client) doGet(ctx context.Context, sandboxIDOrName string) (*Sandbox, error) {
	if sandboxIDOrName == "" {
		return nil, errors.NewNorthraysError("sandbox ID or name is required", 0, nil)
	}
...
func (c *Client) streamBuildLogsToChannel(ctx context.Context, sandboxID string, logChan chan<- string) error {
	...
	if c.jwtToken != "" && c.organizationID != "" {
		httpReq.Header.Set("X-Northrays-Organization-ID", c.organizationID)
	}
