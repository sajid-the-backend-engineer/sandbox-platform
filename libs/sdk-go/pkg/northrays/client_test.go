...
func TestNewClient(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			envVars: map[string]string{
				"NORTHRAYS_API_KEY": "test-api-key",
			},
			expectedError: false,
		...
		},
		{
			...
			envVars: map[string]string{
				"NORTHRAYS_JWT_TOKEN":       "test-jwt-token",
				"NORTHRAYS_ORGANIZATION_ID": "test-org-id",
			},
			expectedError: false,
			validateClient: func(t *testing.T, c *Client) {
		...
		},
		{
			...
			envVars: map[string]string{
				"NORTHRAYS_API_KEY": "test-api-key",
				"NORTHRAYS_API_URL": "https://custom.api.url/api",
			},
			expectedError: false,
			validateClient: func(t *testing.T, c *Client) {
		...
		},
		{
			...
			envVars:       map[string]string{},
			expectedError: true,
			errorContains: "Authentication credentials not found. Set NORTHRAYS_API_KEY, or both NORTHRAYS_JWT_TOKEN and NORTHRAYS_ORGANIZATION_ID",
		},
		{
			...
			expectedError: true,
			errorContains: "NORTHRAYS_ORGANIZATION_ID is required when authenticating with NORTHRAYS_JWT_TOKEN",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("NORTHRAYS_API_KEY", "")
			t.Setenv("NORTHRAYS_API_URL", "")
			t.Setenv("NORTHRAYS_JWT_TOKEN", "")
			t.Setenv("NORTHRAYS_ORGANIZATION_ID", "")
...
func TestNewClientWithConfig(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			envVars: map[string]string{
				"NORTHRAYS_API_KEY": "env-api-key",
				"NORTHRAYS_API_URL": "https://env.api.url/api",
			},
			expectedError: false,
			validateClient: func(t *testing.T, c *Client) {
				assert.Equal(t, "config-api-key", c.apiKey)
...
func TestHandleAPIError(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedType:   "*errors.NorthraysNotFoundError",
		...
		},
		{
			...
			expectedType:   "*errors.NorthraysRateLimitError",
		...
		},
		{
			...
			expectedType:   "*errors.NorthraysError",
...
func TestGet(t *testing.T) {
	t.Run("error when sandbox ID is empty", func(t *testing.T) {
		t.Setenv("NORTHRAYS_API_KEY", "test-api-key")
		t.Setenv("NORTHRAYS_API_URL", "")
		t.Setenv("NORTHRAYS_JWT_TOKEN", "")
		t.Setenv("NORTHRAYS_ORGANIZATION_ID", "")
...
func TestStreamBuildLogsToChannel(t *testing.T) {
	t.Run("streams logs successfully", func(t *testing.T) {
		...
		t.Setenv("NORTHRAYS_API_KEY", "test-api-key")
		t.Setenv("NORTHRAYS_API_URL", server.URL)
		t.Setenv("NORTHRAYS_JWT_TOKEN", "")
		t.Setenv("NORTHRAYS_ORGANIZATION_ID", "")
...
func TestServerURLConstruction(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			expectedURL: "https://app.northrays.com/api",
		},
		{
			name:        "custom API URL with path",
			apiURL:      "https://custom.northrays.com/api/v1",
			expectedURL: "https://custom.northrays.com/api/v1",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Setenv("NORTHRAYS_API_KEY", "test-api-key")
			t.Setenv("NORTHRAYS_API_URL", tt.apiURL)
			t.Setenv("NORTHRAYS_JWT_TOKEN", "")
			t.Setenv("NORTHRAYS_ORGANIZATION_ID", "")
...
func TestJWTAuthentication(t *testing.T) {
	t.Setenv("NORTHRAYS_API_KEY", "")
	t.Setenv("NORTHRAYS_API_URL", "")
	t.Setenv("NORTHRAYS_JWT_TOKEN", "test-jwt-token")
	t.Setenv("NORTHRAYS_ORGANIZATION_ID", "test-org-id")
	...
	cfg := client.apiClient.GetConfig()
	orgHeader, exists := cfg.DefaultHeader["X-Northrays-Organization-ID"]
...
func TestSDKSourceHeader(t *testing.T) {
	...
	cfg := client.apiClient.GetConfig()
	sourceHeader, exists := cfg.DefaultHeader["X-Northrays-Source"]
	assert.True(t, exists)
...
func TestNewClientServerURLFallback(t *testing.T) {
	...
	t.Setenv("NORTHRAYS_JWT_TOKEN", "")
	t.Setenv("NORTHRAYS_ORGANIZATION_ID", "")
	t.Setenv("NORTHRAYS_SERVER_URL", "https://server-url.example/api")
...
func TestHandleAPIErrorUsesOpenAPIBody(t *testing.T) {
	...
	assert.Equal(t, "*errors.NorthraysRateLimitError", fmt.Sprintf("%T", converted))
}
