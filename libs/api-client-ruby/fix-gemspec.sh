...
sed -i "s/\`find \*\`\.split(\"\\\\n\")\.uniq\.sort\.select { |f| !f\.empty? }/\`git ls-files -z\`\.split(\"\\\\x0\")\.reject { |f| f.match(%r{^(test|spec|features)\/}) || f.end_with?('.gem') || f.start_with?('Gemfile') }/g" "$SCRIPT_DIR/northrays_api_client.gemspec"
sed -i "s/\`find spec\/\*\`\.split(\"\\\\n\")/Dir.exist?('spec') ? Dir.glob('spec\/\*\*\/\*') : []/g" "$SCRIPT_DIR/northrays_api_client.gemspec"
