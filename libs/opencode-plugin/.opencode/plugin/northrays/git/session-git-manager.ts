...
import { NorthraysSandboxGitManager } from './sandbox-git-manager'
import { HostGitManager } from './host-git-manager'
import type { PluginInput } from '@opencode-ai/plugin'

/**
 * SessionGitManager: Combines NorthraysSandboxGitManager and HostGitManager for session lifecycle git operations.
 */
export class SessionGitManager {
  private readonly sandboxGit: NorthraysSandboxGitManager
  ...
  constructor(sandbox: Sandbox, repoPath: string, worktree: string, branchNumber: number) {
    ...
    this.sandboxGit = new NorthraysSandboxGitManager(sandbox, repoPath)
  ...
  private async getSshUrl(): Promise<string> {
    ...
    return `ssh://${sshAccess.token}@ssh.app.northrays.com${this.repoPath}`
  }
