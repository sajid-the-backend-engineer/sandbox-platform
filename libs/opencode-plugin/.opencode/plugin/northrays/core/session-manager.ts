...
/**
 * Manages Northrays sandbox sessions and persists session-sandbox mappings
 * Stores data per-project in ~/.local/share/opencode/storage/northrays/{projectId}.json
 */
...
import { NorthraysSandboxGitManager } from '../git/sandbox-git-manager'
import { ProjectDataStorage } from './project-data-storage'
import type { PluginInput } from '@opencode-ai/plugin'
import { toast } from './toast'

export class NorthraysSessionManager {
  ...
  async getSandbox(sessionId: string, projectId: string, worktree: string, pluginCtx?: PluginInput): Promise<Sandbox> {
    ...
    if (!this.apiKey) {
      logger.error('NORTHRAYS_API_KEY is not set. Cannot create or retrieve sandbox.')
      toast.show({
        title: 'Sandbox error',
        message: 'NORTHRAYS_API_KEY is not set. Please set the environment variable to use Northrays sandboxes.',
        variant: 'error',
      })
      throw new Error('NORTHRAYS_API_KEY is not set. Please set the environment variable to use Northrays sandboxes.')
    }
    ...
    if (this.isPartiallyInitialized(existing)) {
      ...
      const northrays = new Northrays({ apiKey: this.apiKey })
      const reconnectStart = Date.now()
      logger.info(`Northrays get begin sandboxId=${existing.id}`)
      const sandbox = await northrays.get(existing.id)
      logger.info(`Northrays get done sandboxId=${existing.id} in ${Date.now() - reconnectStart}ms`)
      ...
      if (!branchNumber) {
        try {
          await new NorthraysSandboxGitManager(sandbox, this.repoPath).ensureDirectory()
        } catch (err) {
    ...
    logger.info(`Northrays create begin sessionId=${sessionId}`)
    const waitingLog = setTimeout(() => {
      logger.warn(`Northrays create still waiting after ${Date.now() - createStart}ms (sessionId=${sessionId})`)
    }, 15_000)
    const sandbox = await northrays.create().finally(() => clearTimeout(waitingLog))
    logger.info(`Northrays create done sessionId=${sessionId} sandboxId=${sandbox.id} in ${Date.now() - createStart}ms`)
    this.sessionSandboxes.set(sessionId, sandbox)
  ...
  async deleteSandbox(sessionId: string, projectId: string): Promise<void> {
    ...
    if (!sandbox || this.isPartiallyInitialized(sandbox)) {
      ...
      if (sessionInfo?.sandboxId) {
        const northrays = new Northrays({ apiKey: this.apiKey })
        try {
          sandbox = await northrays.get(sessionInfo.sandboxId)
          this.sessionSandboxes.set(sessionId, sandbox)
        } catch (err) {
