...
/**
 * Resolve the Northrays API key.
 *
 * Order:
 *   1. `NORTHRAYS_API_KEY` environment variable (the documented SDK convention).
...
export async function resolveApiKey(ctx: ExtensionContext): Promise<string | undefined> {
  const fromEnv = process.env.NORTHRAYS_API_KEY?.trim()
  if (fromEnv) return fromEnv

  if (!ctx.hasUI) return undefined

  const entered = await ctx.ui.input('Northrays API key', 'NORTHRAYS_API_KEY — create one at https://app.northrays.com')
