...
module Northrays
  class LspServer
    ...
    # @return [NorthraysToolboxApiClient::LspApi]
    ...
    # @param toolbox_api [NorthraysToolboxApiClient::LspApi]
    ...
    #
    # @param path [String]
    # @param position [Northrays::LspServer::Position]
    # @return [NorthraysApiClient::CompletionList]
    def completions(path:, position:)
      toolbox_api.completions(
        NorthraysToolboxApiClient::LspCompletionParams.new(
          language_id:,
          path_to_project:,
          uri: uri(path),
          position: NorthraysApiClient::Position.new(line: position.line, character: position.character)
        )
      )
    end
    ...
    def did_close(path)
      toolbox_api.did_close(
        NorthraysToolboxApiClient::LspDocumentRequest.new(language_id:, path_to_project:, uri: uri(path))
      )
    end
    ...
    #
    # @param path [String]
    # @return [Array<NorthraysToolboxApiClient::LspSymbol]
    ...
    def start
      toolbox_api.start(
        NorthraysToolboxApiClient::LspServerRequest.new(language_id:, path_to_project:)
      )
    end
