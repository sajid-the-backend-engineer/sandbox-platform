...
module Northrays
  # Re-export of api-client enum constants under the Northrays namespace so
  # SDK consumers never need to import from NorthraysApiClient directly.
  SandboxClass = NorthraysApiClient::SandboxClass
  SandboxState = NorthraysApiClient::SandboxState
  SandboxListSortField = NorthraysApiClient::SandboxListSortField
  SandboxListSortDirection = NorthraysApiClient::SandboxListSortDirection
  GpuType = NorthraysApiClient::GpuType
  ...
  #
  # @example
  #   query = Northrays::ListSandboxesQuery.new(labels: { 'env' => 'prod' }, limit: 10)
  #   northrays.list(query).each { |sandbox| puts sandbox.id }
