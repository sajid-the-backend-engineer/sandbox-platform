...
module Northrays
  class Git
    ...
    # @return [NorthraysToolboxApiClient::GitApi] API client for Sandbox operations
    attr_reader :toolbox_api

    # Initializes a new Git handler instance.
    #
    # @param sandbox_id [String] The Sandbox ID.
    # @param toolbox_api [NorthraysToolboxApiClient::GitApi] API client for Sandbox operations.
    ...
    # @return [void]
    # @raise [Northrays::Sdk::Error] if adding files fails
    #
    # @example
    #   # Stage a single file
    ...
    #   ])
    def add(path, files)
      toolbox_api.add_files(NorthraysToolboxApiClient::GitAddRequest.new(path:, files:))
    rescue NorthraysToolboxApiClient::ApiError => e
    ...
    # @param path [String] Path to the Git repository root. Relative paths are resolved based on
    #   the sandbox working directory.
    # @return [NorthraysApiClient::ListBranchResponse] List of branches in the repository.
    # @raise [Northrays::Sdk::Error] if listing branches fails
    #
    # @example
    ...
    # @return [void]
    # @raise [Northrays::Sdk::Error] if cloning repository fails
    #
    # @example
    ...
    def clone(url:, path:, branch: nil, commit_id: nil, username: nil, password: nil, insecure_skip_tls: nil) # rubocop:disable Metrics/MethodLength, Metrics/ParameterLists
      toolbox_api.clone_repository(
        NorthraysToolboxApiClient::GitCloneRequest.new(
          url: url,
          branch: branch,
          path: path,
          username: username,
    ...
    # @raise [Northrays::Sdk::Error] if committing changes fails
    #
    # @example
    ...
    def commit(path:, message:, author:, email:, allow_empty: false)
      response = toolbox_api.commit_changes(
        NorthraysToolboxApiClient::GitCommitRequest.new(path:, message:, author:, email:, allow_empty:)
      )
    ...
    # @param password [String, nil] Git password or token for authentication.
    # @return [void]
    # @raise [Northrays::Sdk::Error] if pushing changes fails
    #
    # @example
    ...
    def push(path:, username: nil, password: nil)
      toolbox_api.push_changes(
        NorthraysToolboxApiClient::GitRepoRequest.new(path:, username:, password:)
      )
    rescue NorthraysToolboxApiClient::ApiError => e
    ...
    # @param password [String, nil] Git password or token for authentication.
    # @return [void]
    # @raise [Northrays::Sdk::Error] if pulling changes fails
    #
    # @example
    ...
    # @param path [String] Path to the Git repository root. Relative paths are resolved based on
    #   the sandbox working directory.
    # @return [NorthraysToolboxApiClient::GitStatus] Repository status information including:
    # @raise [Northrays::Sdk::Error] if getting status fails
    #
    # @example
    ...
    # @return [void]
    # @raise [Northrays::Sdk::Error] if checking out branch fails
    #
    # @example
    #   # Checkout a branch
    ...
    def checkout_branch(path, branch)
      toolbox_api.checkout_branch(
        NorthraysToolboxApiClient::GitCheckoutRequest.new(path:, branch:)
      )
    rescue NorthraysToolboxApiClient::ApiError => e
    ...
    # @return [void]
    # @raise [Northrays::Sdk::Error] if creating branch fails
    #
    # @example
    #   # Create a new branch
    ...
    def create_branch(path, name)
      toolbox_api.create_branch(
        NorthraysToolboxApiClient::GitBranchRequest.new(path:, name:)
      )
    rescue NorthraysToolboxApiClient::ApiError => e
    ...
    # @return [void]
    # @raise [Northrays::Sdk::Error] if deleting branch fails
    #
    # @example
    #   # Delete a branch
    ...
    def delete_branch(path, name)
      toolbox_api.delete_branch(
        NorthraysToolboxApiClient::GitDeleteBranchRequest.new(path:, name:)
      )
    rescue NorthraysToolboxApiClient::ApiError => e
