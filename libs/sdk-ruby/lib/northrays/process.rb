...
module Northrays
  class Process # rubocop:disable Metrics/ClassLength
    ...
    # @return [NorthraysToolboxApiClient::ProcessApi] API client for Sandbox operations
    ...
    # @param toolbox_api [NorthraysToolboxApiClient::ProcessApi] API client for Sandbox operations
    ...
    def exec(command:, cwd: nil, env: nil, timeout: nil)
      ...
      response = toolbox_api.execute_command(NorthraysToolboxApiClient::ExecuteRequest.new(command:, cwd:, envs:,
                                                                                         timeout:))
    ...
    def code_run(code:, params: nil, timeout: nil)
      response = toolbox_api.code_run(
        NorthraysToolboxApiClient::CodeRunRequest.new(
    ...
    def create_session(session_id)
      toolbox_api.create_session(NorthraysToolboxApiClient::CreateSessionRequest.new(session_id:))
    end

    # Gets a session in the Sandbox
    #
    # @param session_id [String] Unique identifier of the session to retrieve
    # @return [NorthraysApiClient::Session] Session information including session_id and commands
    #
    # @example
    ...
    #
    # @return [NorthraysApiClient::Session] Entrypoint session information including session_id and commands
    #
    # @example
    ...
    # @return [NorthraysApiClient::Command] Command information including id, command, and exit_code
    #
    # @example
    ...
    # @param req [Northrays::SessionExecuteRequest] Command execution request containing command and run_async
    # @return [Northrays::SessionExecuteResponse] Command execution results containing cmd_id, output, stdout, stderr, and exit_code
    #
    # @example
    ...
    #
    #   # Change directory
    #   req = Northrays::SessionExecuteRequest.new(command: "cd /workspace")
    #   sandbox.process.execute_session_command(session_id:, req:)
    #
    #   # Create a file
    #   req = Northrays::SessionExecuteRequest.new(command: "echo 'Hello' > test.txt")
    #   sandbox.process.execute_session_command(session_id:, req:)
    #
    #   # Read the file
    #   req = Northrays::SessionExecuteRequest.new(command: "cat test.txt")
    ...
    def execute_session_command(session_id:, req:) # rubocop:disable Metrics/MethodLength
      response = toolbox_api.session_execute_command(
        session_id,
        NorthraysToolboxApiClient::SessionExecuteRequest.new(command: req.command, run_async: req.run_async,
    ...
    # @param session_id [String] Unique identifier of the session
    # @param command_id [String] Unique identifier of the command
    # @return [Northrays::SessionCommandLogsResponse] Command logs including output, stdout, and stderr
    #
    # @example
    ...
    def get_session_command_logs_async(session_id:, command_id:, on_stdout:, on_stderr:) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
      ...
      WebSocket::Client::Simple.connect(
        ...
        headers: toolbox_api.api_client.default_headers.dup.merge(
          'X-Northrays-Preview-Token' => preview_link.token,
    ...
    #
    # @return [Northrays::SessionCommandLogsResponse] Entrypoint logs including output, stdout, and stderr
    #
    # @example
    ...
    def send_session_command_input(session_id:, command_id:, data:)
      toolbox_api.send_input(
        session_id,
        command_id,
        NorthraysToolboxApiClient::SessionSendInputRequest.new(data:)
      )
    end

    #
    # @return [Array<NorthraysApiClient::Session>] List of all sessions in the Sandbox
    #
    # @example
    ...
    #   pty_size = Northrays::PtySize.new(rows: 30, cols: 120)
    ...
    #
    # @raise [Northrays::Sdk::Error] If the PTY session creation fails or the session ID is already in use.
    def create_pty_session(id:, cwd: nil, envs: nil, pty_size: nil) # rubocop:disable Metrics/MethodLength
      response = toolbox_api.create_pty_session(
        NorthraysToolboxApiClient::PtyCreateRequest.new(
          id:,
          cwd:,
          envs:,
          cols: pty_size&.cols,
    ...
    #   pty_handle.disconnect
    #
    # @raise [Northrays::Sdk::Error] If the PTY session doesn't exist or connection fails.
    ...
    # @return [NorthraysApiClient::PtySessionInfo] Updated PTY session information
    #
    # @example
    #   pty_size = Northrays::PtySize.new(rows: 30, cols: 120)
    ...
    def resize_pty_session(session_id, pty_size)
      toolbox_api.resize_pty_session(
        session_id,
        NorthraysToolboxApiClient::PtyResizeRequest.new(
          cols: pty_size.cols,
          rows: pty_size.rows
        )
      )
    ...
    #
    # @return [Array<NorthraysApiClient::PtySessionInfo>] List of PTY session information
    #
    # @example
    ...
    # @return [NorthraysApiClient::PtySessionInfo] Detailed information about the PTY session including ID, state,
