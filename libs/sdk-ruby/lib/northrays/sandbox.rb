...
module Northrays
  class Sandbox # rubocop:disable Metrics/ClassLength
    ...
    # @return [NorthraysApiClient::SandboxState] The state of the sandbox
    attr_reader :state

    # @return [NorthraysApiClient::SandboxDesiredState] The desired state of the sandbox
    ...
    # @return [Array<NorthraysApiClient::SandboxVolume>, nil] Volumes attached to the sandbox.
    #   Not returned by list results; call #refresh on each item to populate.
    attr_reader :volumes

    # @return [NorthraysApiClient::BuildInfo, nil] Build information for the sandbox if it was
    ...
    # @return [NorthraysApiClient::SandboxApi]
    attr_reader :sandbox_api

    # @return [Northrays::Process]
    attr_reader :process

    # @return [Northrays::FileSystem]
    attr_reader :fs

    # @return [Northrays::Git]
    attr_reader :git

    # @return [Northrays::ComputerUse]
    attr_reader :computer_use

    # @return [Northrays::CodeInterpreter]
    attr_reader :code_interpreter

    # @params config [Northrays::Config]
    # @params sandbox_api [NorthraysApiClient::SandboxApi]
    # @params sandbox_dto [NorthraysApiClient::Sandbox, NorthraysApiClient::SandboxListItem]
    # @params otel_state [Northrays::OtelState, nil]
    def initialize(sandbox_dto:, config:, sandbox_api:, otel_state: nil) # rubocop:disable Metrics/MethodLength
      ...
      create_authenticated_client = lambda do
        client = NorthraysToolboxApiClient::ApiClient.new(toolbox_api_config)
        client.default_headers['Authorization'] = "Bearer #{config.api_key || config.jwt_token}"
        client.default_headers['X-Northrays-Source'] = 'sdk-ruby'
        client.default_headers['X-Northrays-SDK-Version'] = Sdk::VERSION
        client.default_headers['X-Northrays-Organization-ID'] = config.organization_id if config.jwt_token
        client.user_agent = "sdk-ruby/#{Sdk::VERSION}"
        client
      end

      process_api = NorthraysToolboxApiClient::ProcessApi.new(create_authenticated_client.call)
      fs_api = NorthraysToolboxApiClient::FileSystemApi.new(create_authenticated_client.call)
      git_api = NorthraysToolboxApiClient::GitApi.new(create_authenticated_client.call)
      lsp_api = NorthraysToolboxApiClient::LspApi.new(create_authenticated_client.call)
      computer_use_api = NorthraysToolboxApiClient::ComputerUseApi.new(create_authenticated_client.call)
      interpreter_api = NorthraysToolboxApiClient::InterpreterApi.new(create_authenticated_client.call)
      info_api = NorthraysToolboxApiClient::InfoApi.new(create_authenticated_client.call)
    ...
    def update_network_settings(network_block_all: nil, network_allow_list: nil, domain_allow_list: nil)
      ...
      body = NorthraysApiClient::UpdateSandboxNetworkSettings.new(
        network_block_all:,
        network_allow_list:,
        domain_allow_list:
      )
    ...
    # @return [NorthraysApiClient::SshAccessDto]
    ...
    def labels=(labels)
      @labels = sandbox_api.replace_labels(id, NorthraysApiClient::SandboxLabels.build_from_hash(labels:)).labels
    end
    ...
    #
    # @param port [Integer]
    # @return [NorthraysApiClient::PortPreviewUrl]
    ...
    # @return [NorthraysApiClient::SignedPortPreviewUrl] The signed preview URL response object
    #
    # @example
    ...
    def start(timeout = DEFAULT_TIMEOUT)
      ...
      ) { wait_for_states(operation: OPERATION_START, target_states: [NorthraysApiClient::SandboxState::STARTED]) }
    end
    ...
    #
    # @example
    #   sandbox = northrays.get('my-sandbox-id')
    ...
    def stop(timeout = DEFAULT_TIMEOUT, force: false) # rubocop:disable Metrics/MethodLength
      with_timeout(
        ...
        wait_for_states(
          ...
          target_states: [NorthraysApiClient::SandboxState::STOPPED, NorthraysApiClient::SandboxState::DESTROYED]
        )
      end
    end
    ...
    #
    # @param resources [Northrays::Resources] New resource configuration (cpu, memory, disk only)
    # @param timeout [Numeric] Maximum wait time in seconds (defaults to 60 s)
    # @return [void]
    ...
    #   sandbox.resize(Northrays::Resources.new(cpu: 4, memory: 8))
    #
    # @example Resize a stopped sandbox (CPU, memory, and disk can be changed)
    #   sandbox.stop
    #   sandbox.resize(Northrays::Resources.new(cpu: 2, memory: 4, disk: 30))
    def resize(resources, timeout = DEFAULT_TIMEOUT)
      ...
      with_timeout(
        ...
        setup: proc {
          ...
          resize_request = NorthraysApiClient::ResizeSandbox.new(resize_attrs)
    ...
    def wait_for_resize_complete(_timeout = DEFAULT_TIMEOUT)
      wait_for_states(operation: OPERATION_RESIZE, target_states: [NorthraysApiClient::SandboxState::STARTED,
                                                                   NorthraysApiClient::SandboxState::STOPPED])
    end
    ...
    #
    # @param language_id [Symbol] The language server type (e.g., Northrays::LspServer::Language::PYTHON)
    # @param path_to_project [String] Path to the project root directory. Relative paths are resolved
    #                      based on the sandbox working directory.
    # @return [Northrays::LspServer]
    ...
    #
    # @param token [String]
    # @return [NorthraysApiClient::SshAccessValidationDto]
    ...
    def wait_for_sandbox_stop(_timeout = DEFAULT_TIMEOUT)
      wait_for_states(operation: OPERATION_STOP, target_states: [NorthraysApiClient::SandboxState::STOPPED,
                                                                 NorthraysApiClient::SandboxState::DESTROYED])
    end
    ...
    # @param timeout [Numeric] Maximum wait time in seconds (defaults to 60 s)
    # @return [Northrays::Sandbox] The forked Sandbox
    def experimental_fork(name: nil, timeout: DEFAULT_TIMEOUT) # rubocop:disable Metrics/MethodLength
      ...
      with_timeout(
        ...
        setup: proc {
          forked_dto = sandbox_api.fork_sandbox(id, NorthraysApiClient::ForkSandbox.new(name:))
        }
      ) do
        forked = Sandbox.new(
        ...
        forked.send(:wait_for_states, operation: OPERATION_START,
                                      target_states: [NorthraysApiClient::SandboxState::STARTED])
        return forked
      end
    end
    ...
    def experimental_create_snapshot(name:, timeout: DEFAULT_TIMEOUT)
      with_timeout(
        ...
        setup: proc {
          sandbox_api.create_sandbox_snapshot(id, NorthraysApiClient::CreateSandboxSnapshot.new(name:))
          refresh
        }
    ...
    # @return [NorthraysToolboxApiClient::Configuration]
    def build_toolbox_api_config
      NorthraysToolboxApiClient::Configuration.new.configure do |cfg|
    ...
    def process_response(sandbox_dto) # rubocop:disable Metrics/MethodLength, Metrics/AbcSize
      # Fields shared by both NorthraysApiClient::Sandbox and NorthraysApiClient::SandboxListItem.
      @id = sandbox_dto.id
      ...
      # Fields only present on the full NorthraysApiClient::Sandbox DTO (not returned by list
      # results; call #refresh on each item to populate them).
      return unless sandbox_dto.is_a?(NorthraysApiClient::Sandbox)
    ...
    # @param target_states [Array<NorthraysApiClient::SandboxState>] List of the target states
    # @return [void]
    # @raise [Northrays::Sdk::Error]
    def wait_for_states(operation:, target_states:)
      ...
      loop do
        ...
        when NorthraysApiClient::SandboxState::ERROR, NorthraysApiClient::SandboxState::BUILD_FAILED
    ...
    def wait_for_snapshot_complete
      interval = INITIAL_POLL_INTERVAL
      start_time = ::Process.clock_gettime(::Process::CLOCK_MONOTONIC)
      while state == NorthraysApiClient::SandboxState::SNAPSHOTTING
        refresh

        if [NorthraysApiClient::SandboxState::ERROR, NorthraysApiClient::SandboxState::BUILD_FAILED].include?(state)
          raise Sdk::Error,
                "Sandbox #{id} snapshot failed with state: #{state}, error reason: #{error_reason}"
        end

        break if state != NorthraysApiClient::SandboxState::SNAPSHOTTING
    ...
    def wait_for_pause_complete
      interval = INITIAL_POLL_INTERVAL
      start_time = ::Process.clock_gettime(::Process::CLOCK_MONOTONIC)
      while state == NorthraysApiClient::SandboxState::PAUSING
        refresh
        ...
        break if state != NorthraysApiClient::SandboxState::PAUSING
