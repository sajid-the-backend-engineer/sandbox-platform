...
module Northrays
  class SnapshotService
    ...
    # @param snapshots_api [NorthraysApiClient::SnapshotsApi] The snapshots API client
    # @param object_storage_api [NorthraysApiClient::ObjectStorageApi] The object storage API client
    ...
    # @return [Northrays::PaginatedResource] Paginated list of all Snapshots
    ...
    # Delete a Snapshot.
    #
    # @param snapshot [Northrays::Snapshot] Snapshot to delete
    # @return [void]
    #
    # @example
    #   northrays = Northrays::Northrays.new
    ...
    # @return [Northrays::Snapshot] The Snapshot object
    #
    # @example
    #   northrays = Northrays::Northrays.new
    #   snapshot = northrays.snapshot.get("demo")
    ...
    #
    # @param params [Northrays::CreateSnapshotParams] Parameters for snapshot creation
    # @param on_logs [Proc, Nil] Callback proc handling snapshot creation logs
    # @return [Northrays::Snapshot] The created snapshot
    #
    # @example
    ...
    def create(params, on_logs: nil) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
      create_snapshot_req = NorthraysApiClient::CreateSnapshot.new(name: params.name)

      if params.image.is_a?(String)
        create_snapshot_req.image_name = params.image
        create_snapshot_req.entrypoint = params.entrypoint
      else
        create_snapshot_req.build_info = NorthraysApiClient::CreateBuildInfo.new(
      ...
      if [NorthraysApiClient::SnapshotState::ERROR, NorthraysApiClient::SnapshotState::BUILD_FAILED].include?(snapshot.state)
    ...
    # Activate a snapshot
    #
    # @param snapshot [Northrays::Snapshot] The snapshot instance
    # @return [Northrays::Snapshot]
    ...
    #
    # @param image [Northrays::Image] The Image instance
    ...
    # @return [NorthraysApiClient::SnapshotsApi] The snapshots API client
    attr_reader :snapshots_api

    # @return [NorthraysApiClient::ObjectStorageApi, nil] The object storage API client
    ...
    #
    # @param snapshot [NorthraysApiClient::SnapshotDto]
    # @param on_logs [Proc, nil]
    # @return [NorthraysApiClient::SnapshotDto]
    def wait_for_snapshot(snapshot, on_logs:) # rubocop:disable Metrics/AbcSize, Metrics/MethodLength
      terminal_states = [
        NorthraysApiClient::SnapshotState::ACTIVE,
        NorthraysApiClient::SnapshotState::ERROR,
        NorthraysApiClient::SnapshotState::BUILD_FAILED
      ]
      ...
      until terminal_states.include?(snapshot.state)
        ...
        if on_logs && thread.nil? && snapshot.state != NorthraysApiClient::SnapshotState::PENDING
        ...
        if on_logs && previous_state != snapshot.state
          if snapshot.state != NorthraysApiClient::SnapshotState::PENDING && thread.nil?
            thread = start_log_streaming(snapshot, on_logs:)
          end
          on_logs.call("Creating snapshot #{snapshot.name} (#{snapshot.state})")
        end
      ...
      if on_logs && snapshot.state == NorthraysApiClient::SnapshotState::ACTIVE
