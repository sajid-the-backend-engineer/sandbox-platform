...
module Northrays
  class FileSystem # rubocop:disable Metrics/ClassLength
    ...
    # @return [NorthraysToolboxApiClient::FileSystemApi] API client for Sandbox operations
    attr_reader :toolbox_api

    # Initializes a new FileSystem instance.
    #
    # @param sandbox_id [String] The Sandbox ID
    # @param toolbox_api [NorthraysToolboxApiClient::FileSystemApi] API client for Sandbox operations
    ...
    #   on the sandbox working directory.
    # @return [NorthraysApiClient::FileInfo] Detailed file information
    # @raise [Northrays::Sdk::Error] If the operation fails
    #
    # @example
    #   # Get file metadata
    ...
    # @return [Array<NorthraysApiClient::FileInfo>] List of file and directory information
    # @raise [Northrays::Sdk::Error] If the operation fails
    #
    # @example
    ...
    # @param on_progress [Proc, nil] Optional callback invoked with a Northrays::DownloadProgress
    ...
    #   during streaming, the next chunk raises Northrays::Sdk::Error and the underlying HTTP
    ...
    # @raise [Northrays::Sdk::Error] If the file does not exist, the operation fails, or
    ...
    def upload_file(source, remote_path)
      if source.is_a?(String) && File.exist?(source)
        ...
        tmp = Tempfile.new('northrays_upload')
        begin
          tmp.binmode
    ...
    #   +Northrays::UploadProgress+ struct as libcurl reports bytes actually uploaded.
    ...
    #   Northrays::Sdk::Error and the in-progress upload is aborted (no destination file
    #   is left on the sandbox thanks to the daemon's atomic-rename behaviour).
    # @return [void]
    # @raise [Northrays::Sdk::Error] If the operation fails or +cancel_event+ is set.
    #
    # @example
    ...
    # @return [Array<NorthraysApiClient::Match>] List of matches found in files
    # @raise [Northrays::Sdk::Error] If the operation fails
    #
    # @example
    ...
    # @return [NorthraysApiClient::SearchFilesResponse]
    # @raise [Northrays::Sdk::Error] If the operation fails
    #
    # @example
    #   # Find all Ruby files
    ...
    # @return [Array<NorthraysApiClient::ReplaceResult>] List of results indicating replacements made in each file
    # @raise [Northrays::Sdk::Error] If the operation fails
    #
    # @example
    ...
    def replace_in_files(files:, pattern:, new_value:)
      replace_request = NorthraysApiClient::ReplaceRequest.new(
        files: files,
        pattern: pattern,
        new_value: new_value
      )
