...
module Northrays
  class Northrays
    ...
    # @return [NorthraysApiClient]
    attr_reader :api_client

    # @return [NorthraysApiClient::SandboxApi]
    attr_reader :sandbox_api

    # @return [Northrays::VolumeService]
    attr_reader :volume

    # @return [NorthraysApiClient::ObjectStorageApi]
    attr_reader :object_storage_api

    # @return [NorthraysApiClient::SnapshotsApi]
    attr_reader :snapshots_api

    # @return [Northrays::SnapshotService]
    attr_reader :snapshot

    # @param config [Northrays::Config] Configuration options. Defaults to Northrays::Config.new
    def initialize(config = Config.new)
      ...
      otel_enabled = config.otel_enabled ||
                     ...
                     config.read_env('NORTHRAYS_OTEL_ENABLED') == 'true' ||
                     config.read_env('NORTHRAYS_EXPERIMENTAL_OTEL_ENABLED') == 'true'
      @otel_state = (::Northrays.init_otel(Sdk::VERSION) if otel_enabled)

      @api_client = build_api_client
      @sandbox_api = NorthraysApiClient::SandboxApi.new(api_client)
      @config_api = NorthraysApiClient::ConfigApi.new(api_client)
      @volume = VolumeService.new(NorthraysApiClient::VolumesApi.new(api_client), otel_state:)
      @object_storage_api = NorthraysApiClient::ObjectStorageApi.new(api_client)
      @snapshots_api = NorthraysApiClient::SnapshotsApi.new(api_client)
    ...
    # @return [void]
    def close
      ::Northrays.shutdown_otel(@otel_state)
      @otel_state = nil
    end

    # Creates a sandbox with the specified parameters
    #
    # @param params [Northrays::CreateSandboxFromSnapshotParams, Northrays::CreateSandboxFromImageParams, Nil] Sandbox creation parameters
    # @return [Northrays::Sandbox] The created sandbox
    # @raise [Northrays::Sdk::Error] If auto_stop_interval or auto_archive_interval is negative
    ...
    # Deletes a Sandbox.
    #
    # @param sandbox [Northrays::Sandbox]
    # @return [void]
    ...
    #
    # @param query [Northrays::ListSandboxesQuery, nil] Optional filters, sorting, and per-page size.
    # @return [Enumerator<Northrays::Sandbox>]
    ...
    instrument :create, :delete, :get, :start, :stop, component: 'Northrays'
    ...
    # ("Northrays.list_fetch_page") so that paginated iteration emits N spans
    # for N pages.
    #
    # @param q [Northrays::ListSandboxesQuery]
    # @param cursor [String, nil]
    # @return [NorthraysApiClient::ListSandboxesResponse]
    ...
    instrument :fetch_sandbox_page, component: 'Northrays.list'

    # Creates a sandbox with the specified parameters
    #
    # @param params [Northrays::CreateSandboxFromSnapshotParams, Northrays::CreateSandboxFromImageParams] Sandbox creation parameters
    # @param timeout [Numeric] Maximum wait time in seconds (defaults to 60 s).
    ...
    def _create(params, timeout: 60, on_snapshot_create_logs: nil)
      ...
      create_sandbox = NorthraysApiClient::CreateSandbox.new(
        user: params.os_user,
        env: params.env_vars || {},
        labels: labels,
      ...
      if params.respond_to?(:image) && params.image.is_a?(String)
        create_sandbox.build_info = NorthraysApiClient::CreateBuildInfo.new(
      ...
      if response.state == NorthraysApiClient::SandboxState::PENDING_BUILD && on_snapshot_create_logs
        # Wait for state to change from PENDING_BUILD before fetching logs
        while response.state == NorthraysApiClient::SandboxState::PENDING_BUILD
          sleep(1)
      ...
      if sandbox.state != NorthraysApiClient::SandboxState::STARTED
    ...
    def ensure_access_token_defined
      ...
      unless config.jwt_token
        raise Sdk::Error,
              'Authentication credentials not found. Set NORTHRAYS_API_KEY, or both NORTHRAYS_JWT_TOKEN and ' \
              'NORTHRAYS_ORGANIZATION_ID. These can also be provided via Northrays::Config.'
      end
      return if config.organization_id

      raise Sdk::Error,
            'NORTHRAYS_ORGANIZATION_ID is required when authenticating with NORTHRAYS_JWT_TOKEN. ' \
            'It can also be provided via Northrays::Config.'
    end

    # @return [NorthraysApiClient::ApiClient]
    def build_api_client
      NorthraysApiClient::ApiClient.new(api_client_config).tap do |client|
    ...
    # @return [NorthraysApiClient::Configuration]
    def api_client_config
      NorthraysApiClient::Configuration.new.configure do |api_config|
    ...
    # @param sandbox_dto [NorthraysApiClient::Sandbox, NorthraysApiClient::SandboxListItem]
    # @return [Northrays::Sandbox]
    ...
    HEADER_SOURCE = 'X-Northrays-Source'
    private_constant :HEADER_SOURCE

    HEADER_SDK_VERSION = 'X-Northrays-SDK-Version'
    private_constant :HEADER_SDK_VERSION

    HEADER_ORGANIZATION_ID = 'X-Northrays-Organization-ID'
