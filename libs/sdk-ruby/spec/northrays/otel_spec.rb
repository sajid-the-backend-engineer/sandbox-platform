...
RSpec.describe Northrays do
  describe Northrays::OtelState do
  ...
  describe '.shutdown_otel' do
    ...
    it 'delegates to the state shutdown method' do
      state = instance_double(Northrays::OtelState)
  ...
  describe '.with_instrumentation' do
    ...
    it 'records a successful duration metric' do
      ...
      state = instance_double(Northrays::OtelState, tracer: tracer)
  ...
  describe Northrays::Instrumentation do
    let(:test_class) do
      Class.new do
        include Northrays::Instrumentation
    ...
    it 'wraps public methods with Northrays.with_instrumentation' do
      otel_state = double('OtelState')
      instance = test_class.new(otel_state)
      allow(Northrays).to receive(:with_instrumentation).and_yield

      expect(instance.ping('value')).to eq('pong value')
      expect(Northrays).to have_received(:with_instrumentation).with(otel_state, 'Dummy', 'ping')
    end
