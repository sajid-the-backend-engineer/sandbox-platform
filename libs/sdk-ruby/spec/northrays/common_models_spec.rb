...
RSpec.describe 'Northrays common models' do
  it 'stores file upload source and destination' do
    upload = Northrays::FileUpload.new('hello', '/tmp/hello.txt')
  ...
  it 'stores execute response attributes' do
    artifacts = Northrays::ExecutionArtifacts.new('stdout', ['chart'])
    response = Northrays::ExecuteResponse.new(exit_code: 0, result: 'stdout', artifacts: artifacts,
  ...
  it 'defaults execution artifacts to empty stdout and charts' do
    artifacts = Northrays::ExecutionArtifacts.new
  ...
  it 'stores code run params' do
    params = Northrays::CodeRunParams.new(argv: ['-m', 'app'], env: { 'DEBUG' => '1' })
  ...
  it 'stores session execute request flags' do
    req = Northrays::SessionExecuteRequest.new(command: 'bundle exec rspec', run_async: true, suppress_input_echo: true)
  ...
  it 'provides defaults for session execute response' do
    response = Northrays::SessionExecuteResponse.new
  ...
  it 'stores session command logs response fields' do
    response = Northrays::SessionCommandLogsResponse.new(output: 'all', stdout: 'out', stderr: 'err')
  ...
  it 'stores output messages and execution errors' do
    msg = Northrays::OutputMessage.new(output: 'hello')
    error = Northrays::ExecutionError.new(name: 'ValueError', value: 'bad', traceback: 'trace')
  ...
  it 'stores execution results' do
    result = Northrays::ExecutionResult.new(stdout: 'out', stderr: 'err', error: :boom)
  ...
  it 'stores resource values and compacts nils' do
    resources = Northrays::Resources.new(cpu: 2, memory: 4)

    expect(resources.to_h).to eq(cpu: 2, memory: 4)
  end

  it 'stores paginated resources' do
    paginated = Northrays::PaginatedResource.new(items: [1, 2], page: 2, total: 5, total_pages: 3)
  ...
  it 'stores git commit response sha' do
    response = Northrays::GitCommitResponse.new(sha: 'abc123')
