...
RSpec.describe Northrays::LspServer do
  let(:toolbox_api) { instance_double(NorthraysToolboxApiClient::LspApi) }
  let(:lsp) do
    described_class.new(
      language_id: :python,
  ...
  describe '#completions' do
    it 'returns completion list for a position' do
      ...
      result = lsp.completions(path: '/workspace/main.py',
                               position: Northrays::LspServer::Position.new(line: 10,
                                                                          character: 5))
  ...
  describe 'Language constants' do
    it 'defines supported language constants' do
      expect(Northrays::LspServer::Language::ALL).to eq(%i[javascript python typescript])
    end
  end

  describe 'Position' do
    it 'is a data class with line and character' do
      pos = Northrays::LspServer::Position.new(line: 5, character: 10)
