...
ASYNC_MODULE = "northrays._async.northrays"


def _make_async_northrays(config=None):
    from northrays._async.northrays import AsyncNorthrays
    from northrays_api_client_async import Configuration
    ...
    with (
        ...
        return AsyncNorthrays(config)


class TestAsyncNorthraysInit:
    def test_init_with_config(self):
        northrays = _make_async_northrays(NorthraysConfig(api_key="test-key", api_url="https://api.test.io", target="us"))
    ...
    def test_init_with_env_vars(self, env_with_api_key):
        northrays = _make_async_northrays()
    ...
    def test_jwt_without_organization_id_raises(self):
        with pytest.raises(NorthraysAuthenticationError, match="NORTHRAYS_ORGANIZATION_ID is required"):
            _make_async_northrays(NorthraysConfig(jwt_token="jwt", api_url="https://api.test.io", target="us"))


class TestAsyncNorthraysContextManager:
    @pytest.mark.asyncio
    ...
    async def test_close_shuts_down_tracer_provider(self, env_with_api_key):
        ...
        northrays._tracer_provider = tracer_provider

        await northrays.close()

        tracer_provider.shutdown.assert_called_once()
        northrays._api_client.close.assert_awaited_once()
        northrays._toolbox_api_client.close.assert_awaited_once()


class TestAsyncNorthraysSharedSession:
    ...
    async def test_first_request_propagates_session_to_other_rest_client(self, monkeypatch):
        ...
        from northrays.internal import shared_session as ss
        from northrays.internal.shared_session import SharedAiohttpSession
    ...
    def test_retry_backoff_has_jitter_and_grows_with_attempt(self):
        ...
        from northrays.internal.shared_session import (
...
class TestAsyncNorthraysCreateValidation:
    @pytest.mark.asyncio
    async def test_negative_timeout_raises(self, env_with_api_key):
        ...
        with pytest.raises(NorthraysValidationError, match="Timeout must be a non-negative number"):
            await northrays._create(CreateSandboxFromSnapshotParams(language="python"), timeout=-1)
    ...
    async def test_negative_auto_stop_raises(self, env_with_api_key):
        ...
        with pytest.raises(NorthraysValidationError, match="auto_stop_interval must be a non-negative"):
            await northrays._create(CreateSandboxFromSnapshotParams(language="python", auto_stop_interval=-1), timeout=60)
    ...
    async def test_create_defaults_language_and_sets_label(self, env_with_api_key, sandbox_dto):
        ...
        northrays = _make_async_northrays()
        northrays._sandbox_api.create_sandbox = AsyncMock(return_value=sandbox_dto)
...
class TestAsyncNorthraysGetAndList:
    @pytest.mark.asyncio
    ...
    async def test_get_returns_sandbox(self, env_with_api_key, sandbox_dto):
        ...
        northrays = _make_async_northrays()
        northrays._sandbox_api = AsyncMock()
        northrays._sandbox_api.get_sandbox.return_value = sandbox_dto
        sandbox = await northrays.get("test-sandbox-id")
    ...
    def test_list_returns_async_iterator(self, env_with_api_key):
        ...
        assert inspect.isasyncgenfunction(AsyncNorthrays.list)
    ...
    async def test_list_serializes_labels(self, env_with_api_key, sandbox_dto):
        ...
        northrays = _make_async_northrays()
        northrays._sandbox_api.list_sandboxes = AsyncMock(return_value=response)

        sandboxes = []
        async for sb in northrays.list(ListSandboxesQuery(labels={"project": "test"}, limit=10)):
            sandboxes.append(sb)
    ...
    async def test_list_paginates_via_cursor(self, env_with_api_key, sandbox_dto):
        ...
        northrays = _make_async_northrays()
        northrays._sandbox_api.list_sandboxes = AsyncMock(side_effect=[page1, page2])
    ...
    async def test_list_early_termination_stops_fetching(self, env_with_api_key, sandbox_dto):
        ...
        async for _ in northrays.list():
            break

        assert northrays._sandbox_api.list_sandboxes.call_count == 1


class TestAsyncNorthraysValidateLanguageLabel:
    ...
    def test_valid_language_values(self, env_with_api_key, value):
        northrays = _make_async_northrays()
        result = northrays._validate_language_label(value)
...
class TestResolveHappyEyeballsDelay:
    ...
    def test_unset_returns_missing_sentinel(self, raw):
        from northrays._async.northrays import _MISSING_HAPPY_EYEBALLS_DELAY, _resolve_happy_eyeballs_delay
    ...
    def test_none_disables(self, raw):
        from northrays._async.northrays import _resolve_happy_eyeballs_delay
    ...
    def test_invalid_values_raise(self, raw):
        ...
        with pytest.raises(
            ...
            match="NORTHRAYS_HAPPY_EYEBALLS_DELAY must be a finite non-negative float or 'none'",
        ):
