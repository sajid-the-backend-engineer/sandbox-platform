...
"""
End-to-end tests for the async SDK (``AsyncNorthrays``).

Mirrors ``test_e2e.py`` but exercises the asyncio code path.  Run with::

    NORTHRAYS_API_KEY=dtn_... NORTHRAYS_API_URL=https://app.northrays.com/api \\
...
from northrays import AsyncNorthrays, CreateSandboxFromSnapshotParams, ListSandboxesQuery
from northrays.common.errors import NorthraysConnectionError, NorthraysError, NorthraysNotFoundError
...
# so the module-scoped ``async_northrays_client`` / ``async_sandbox`` fixtures
...
async def async_northrays_client() -> AsyncIterator[AsyncNorthrays]:
    async with AsyncNorthrays() as northrays:
        yield northrays


@pytest_asyncio.fixture(loop_scope="module", scope="module")
async def async_sandbox(async_northrays_client: AsyncNorthrays):
    params = CreateSandboxFromSnapshotParams(language="python")
    sb = await async_northrays_client.create(params, timeout=120)
    try:
        yield sb
    finally:
        try:
            await async_northrays_client.delete(sb)
        except Exception:
            pass
...
# ===========================================================================
# AsyncNorthrays Client Operations
# ===========================================================================


async def test_async_get_sandbox_by_id(async_northrays_client, async_sandbox):
    fetched = await async_northrays_client.get(async_sandbox.id)
    assert fetched.id == async_sandbox.id
    assert fetched.name == async_sandbox.name


async def test_async_list_sandboxes_contains_created(async_northrays_client, async_sandbox):
    sandboxes = [s async for s in async_northrays_client.list()]
...
async def test_async_list_with_pagination(async_northrays_client, async_sandbox):
    yielded = 0
    async for _ in async_northrays_client.list(ListSandboxesQuery(limit=1)):
        yielded += 1
        if yielded >= 1:
            break
    assert yielded >= 1
...
async def test_async_get_unknown_sandbox_raises_not_found(async_northrays_client):
    name = f"async-e2e-missing-{uuid.uuid4().hex[:12]}"
    with pytest.raises(NorthraysNotFoundError):
        await async_northrays_client.get(name)
...
# ===========================================================================
#
# Hammers ``northrays.get()`` with names that do not exist and verifies that no
# transient connection errors leak through as ``NorthraysConnectionError`` or
# the generic ``NorthraysError``.  Guards against regressions of the retry
...
async def _get_nonexistent(northrays: AsyncNorthrays, sem: asyncio.Semaphore) -> str:
    async with sem:
        name = f"conn-test-{uuid.uuid4().hex[:12]}"
        try:
            await northrays.get(name)
            return "unexpected_found"
        except NorthraysNotFoundError:
            return "not_found"
        except NorthraysConnectionError as e:
            return f"conn_error:{type(e).__name__}:{e}"
        except NorthraysError as e:
            return f"northrays_error:{type(e).__name__}:{e}"
        except Exception as e:  # pragma: no cover - last-resort catch-all
            return f"other:{type(e).__name__}:{e}"


async def test_async_concurrent_get_no_connection_errors(async_northrays_client):
    sem = asyncio.Semaphore(_CONCURRENCY)

    t0 = time.monotonic()
    outcomes = await asyncio.gather(*[_get_nonexistent(async_northrays_client, sem) for _ in range(_ROUNDS)])
    ...
    leaked = results.get("conn_error", 0) + results.get("northrays_error", 0) + results.get("other", 0)
