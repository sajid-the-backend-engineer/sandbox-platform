...
from northrays.common.image import SUPPORTED_PYTHON_SERIES, Image
...
class TestImageDebianSlim:
    ...
    def test_unsupported_version(self):
        with pytest.raises(NorthraysError, match="Invalid Python version"):
...
class TestImagePipInstall:
    ...
    def test_with_index_url(self):
        img = Image.base("python:3.12").pip_install("private-pkg", index_url="https://my-pypi.example.com/simple")
        assert "--index-url" in img.dockerfile()

    def test_with_extra_index_urls(self):
        img = Image.base("python:3.12").pip_install("pkg", extra_index_urls=["https://extra1.example.com/simple"])
        assert "--extra-index-url" in img.dockerfile()

    def test_with_find_links(self):
        img = Image.base("python:3.12").pip_install("pkg", find_links=["https://download.example.com/wheels"])
...
class TestImagePipInstallFromRequirements:
    ...
    def test_nonexistent_requirements(self):
        with pytest.raises(NorthraysError, match="does not exist"):
...
class TestImageWorkdir:
    def test_set_workdir(self):
        img = Image.base("python:3.12").workdir("/home/northrays")
        assert "WORKDIR" in img.dockerfile()
        assert "/home/northrays" in img.dockerfile()
...
class TestImageDockerfileCommands:
    def test_add_dockerfile_commands(self):
        img = Image.base("python:3.12").dockerfile_commands(["RUN echo 'Hello, world!'", "EXPOSE 8080"])
        df = img.dockerfile()
...
class TestImageAddLocal:
    ...
    def test_add_local_file_appends_filename_when_remote_path_is_dir(self):
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            f.write("test content")
            f.flush()
            try:
                img = Image.base("python:3.12").add_local_file(f.name, "/app/")
