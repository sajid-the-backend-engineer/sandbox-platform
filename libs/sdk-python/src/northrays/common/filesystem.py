...
from .errors import NorthraysError, NorthraysValidationError, create_northrays_error
...
class CancelEvent(Protocol):
    ...
    ``NorthraysError``, closing the underlying HTTP connection."""
...
def create_file_download_error(response: FileDownloadResponse) -> NorthraysError:
    """Create the appropriate Northrays exception for a failed file download response."""

    if response.error is None:
        raise NorthraysValidationError("response.error must not be None")

    if response.error_details is None:
        return NorthraysError(response.error)

    return create_northrays_error(
...
def raise_if_stream_error(
    ...
    if not received_file_data:
        raise NorthraysError(f"No file data received for: {remote_path}")
