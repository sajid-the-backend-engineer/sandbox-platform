...
class CodeLanguage(str, Enum):
    """Programming languages supported by Northrays
...
class NorthraysConfig(BaseModel):
    ...
    Attributes:
        api_key (str | None): API key for authentication with the Northrays API. If not set, it must be provided
            via the environment variable `NORTHRAYS_API_KEY`, or a JWT token must be provided instead.
        jwt_token (str | None): JWT token for authentication with the Northrays API. If not set, it must be provided
        ...
        api_url (str | None): URL of the Northrays API. Defaults to `'https://app.northrays.com/api'` if not set
            here or in the environment variable `NORTHRAYS_API_URL`.
        ...
        target (str | None): Target runner location for the Sandbox. Default region for the organization is used
            if not set here or in the environment variable `NORTHRAYS_TARGET`.
        ...
        otel_enabled (bool | None): Enable OpenTelemetry tracing for SDK operations. Defaults
            to `None`, which falls back to the `NORTHRAYS_OTEL_ENABLED` environment variable.
