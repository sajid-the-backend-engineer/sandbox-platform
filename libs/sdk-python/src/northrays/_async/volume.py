...
from northrays_api_client_async import CreateVolume, VolumesApi
...
class AsyncVolumeService:
    ...
    async def list(self) -> list[Volume]:
        ...
        Example:
            ...
            async with AsyncNorthrays() as northrays:
                volumes = await northrays.volume.list()
    ...
    async def get(self, name: str, create: bool = False) -> Volume:
        ...
        Example:
            ```python
            async with AsyncNorthrays() as northrays:
                volume = await northrays.volume.get("test-volume-name", create=True)
                print(f"{volume.name} ({volume.id})")
            ```
        """
        try:
    ...
    async def create(self, name: str) -> Volume:
        ...
        Example:
            ```python
            async with AsyncNorthrays() as northrays:
                volume = await northrays.volume.create("test-volume")
    ...
    async def delete(self, volume: Volume) -> None:
        ...
        Example:
            ```python
            async with AsyncNorthrays() as northrays:
                volume = await northrays.volume.get("test-volume")
