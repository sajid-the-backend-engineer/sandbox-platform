...
from northrays_api_client_async import (
    ApiClient,
    ConfigApi,
    Configuration,
    CreateBuildInfo,
...
from northrays_api_client_async import VolumesApi as VolumesApi
from northrays_toolbox_api_client_async import ApiClient as ToolboxApiClient

from .._utils.enum import to_enum
from .._utils.env import NorthraysEnvReader
...
from ..common.errors import NorthraysAuthenticationError, NorthraysValidationError
...
def _resolve_happy_eyeballs_delay(raw: str | None) -> object:
    """Parse ``NORTHRAYS_HAPPY_EYEBALLS_DELAY`` into the value forwarded to
    ...
    - anything else (NaN, +/-inf, negative, non-numeric, …) →
      :class:`NorthraysValidationError`
    """
    ...
    if value.lower() == "none":
        return None
    invalid_message = f"NORTHRAYS_HAPPY_EYEBALLS_DELAY must be a finite non-negative float or 'none', got {raw!r}"
    try:
        parsed = float(value)
    except ValueError as exc:
        raise NorthraysValidationError(invalid_message) from exc
    ...
    if not math.isfinite(parsed) or parsed < 0:
        raise NorthraysValidationError(invalid_message)
    return parsed


class AsyncNorthrays:
    """Main class for interacting with the Northrays API.

    This class provides asynchronous methods to create, manage, and interact with Northrays Sandboxes.
    ...
    def __init__(self, config: NorthraysConfig | None = None):
        """Initializes Northrays instance with optional configuration.

        If no config is provided, reads from environment variables:
        - `NORTHRAYS_API_KEY`: Required API key for authentication
        - `NORTHRAYS_API_URL`: Required api URL
        - `NORTHRAYS_TARGET`: Optional target environment (if not provided, default region for the organization is used)

        Args:
            config (NorthraysConfig | None): Object containing api_key, api_url, and target.

        Raises:
            NorthraysError: If API key is not provided either through config or environment variables
        ...
        default_api_url = "https://app.northrays.com/api"
        ...
        env_reader: NorthraysEnvReader | None = None
        ...
        if config is None or (
            ...
            env_reader = NorthraysEnvReader()
            self._api_key = self._api_key or (env_reader.get("NORTHRAYS_API_KEY") if not self._jwt_token else None)
            self._jwt_token = self._jwt_token or env_reader.get("NORTHRAYS_JWT_TOKEN")
            self._organization_id = self._organization_id or env_reader.get("NORTHRAYS_ORGANIZATION_ID")
            api_url = api_url or env_reader.get("NORTHRAYS_API_URL") or env_reader.get("NORTHRAYS_SERVER_URL")
            self._target = self._target or env_reader.get("NORTHRAYS_TARGET")

            if env_reader.get("NORTHRAYS_SERVER_URL") and not env_reader.get("NORTHRAYS_API_URL"):
                warnings.warn(
                    "Environment variable `NORTHRAYS_SERVER_URL` is deprecated and will be removed in future versions. "
                    + "Use `NORTHRAYS_API_URL` instead.",
                    DeprecationWarning,
                    stacklevel=2,
                )
        ...
        if not self._api_key and not self._jwt_token:
            msg = (
                ...
                + " Set NORTHRAYS_API_KEY, or both NORTHRAYS_JWT_TOKEN and NORTHRAYS_ORGANIZATION_ID."
                + " These can also be provided via NorthraysConfig."
            )
            raise NorthraysAuthenticationError(msg)
        ...
        # ``NORTHRAYS_HAPPY_EYEBALLS_DELAY`` lets callers tune (or fully disable)
        ...
        env_reader = env_reader or NorthraysEnvReader()
        session_kwargs: dict[str, float | None] = {}
        happy_eyeballs_delay = _resolve_happy_eyeballs_delay(env_reader.get("NORTHRAYS_HAPPY_EYEBALLS_DELAY"))
        ...
        self._api_client.default_headers["X-Northrays-Source"] = "sdk-python-async"

        # Get SDK version dynamically
        try:
            sdk_version = None
            for pkg_name in ["northrays", "northrays_sdk"]:
                try:
            ...
            sdk_version = "unknown"
        self._api_client.default_headers["X-Northrays-SDK-Version"] = sdk_version
        self._api_client.user_agent = f"sdk-python-async/{sdk_version}"

        if not self._api_key:
            if not self._organization_id:
                raise NorthraysAuthenticationError(
                    "NORTHRAYS_ORGANIZATION_ID is required when authenticating with NORTHRAYS_JWT_TOKEN."
                    + " It can also be provided via NorthraysConfig."
                )
            self._api_client.default_headers["X-Northrays-Organization-ID"] = self._organization_id
        ...
        otel_enabled = (
            ...
            or env_reader.get("NORTHRAYS_OTEL_ENABLED") == "true"
            or env_reader.get("NORTHRAYS_EXPERIMENTAL_OTEL_ENABLED") == "true"
        )
        if otel_enabled:
        ...
        resource = Resource.create(
            {
                ...
                service_attributes.SERVICE_NAME: "northrays-python-sdk",
            }
        )
    ...
    async def close(self):
        ...
        This method should be called when you're done using the AsyncNorthrays instance
        ...
        *,
        ...
        Args:
            params (CreateSandboxFromSnapshotParams | None): Parameters for Sandbox creation. If not provided,
                   defaults to default Northrays snapshot and Python language.
        ...
        Raises:
            NorthraysError: If timeout, auto_stop_interval or auto_archive_interval is negative;
    ...
    async def create(
        ...
        *,
        ...
        """Creates Sandboxes from specified image available on some registry or declarative Northrays Image.
        You can specify various parameters, including resources, language, image, environment variables,
        and volumes. Northrays creates snapshot from provided image and uses it to create Sandbox.
    ...
    async def _create(
        ...
        *,
        ...
        if timeout and timeout < 0:
            raise NorthraysValidationError("Timeout must be a non-negative number")

        if params.auto_stop_interval is not None and params.auto_stop_interval < 0:
            raise NorthraysValidationError("auto_stop_interval must be a non-negative integer")

        if params.auto_archive_interval is not None and params.auto_archive_interval < 0:
            raise NorthraysValidationError("auto_archive_interval must be a non-negative integer")
    ...
    async def delete(self, sandbox: AsyncSandbox, timeout: float = 60) -> None:
        ...
        Raises:
            NorthraysError: If sandbox fails to delete or times out

        Example:
            ```python
            sandbox = await northrays.create()
            # ... use sandbox ...
            await northrays.delete(sandbox)  # Clean up when done
            ```
        """
    ...
    async def get(self, sandbox_id_or_name: str) -> AsyncSandbox:
        ...
        Raises:
            NorthraysError: If sandbox_id_or_name is not provided.

        Example:
            ```python
            sandbox = await northrays.get("my-sandbox-id-or-name")
            print(sandbox.state)
            ```
        """
        if not sandbox_id_or_name:
            raise NorthraysValidationError("sandbox_id_or_name is required")
    ...
    async def list(
        ...
        Example:
            ...
            async for sandbox in northrays.list(ListSandboxesQuery(labels={"env": "dev"})):
                print(sandbox.id)
            ```
        """
        ...
        if q.limit is not None and q.limit < 1:
            raise NorthraysValidationError("limit must be a positive integer")
    ...
    @with_instrumentation(name="AsyncNorthrays.list.fetch_page")
    ...
    def _validate_language_label(self, language: str | None = None) -> CodeLanguage:
        ...
        Raises:
            NorthraysError: If the language is not supported.
        """
        if not language:
        ...
        if enum_language is None:
            raise NorthraysValidationError(f"Invalid {CODE_TOOLBOX_LANGUAGE_LABEL}: {language}")
    ...
    async def start(self, sandbox: AsyncSandbox, timeout: float = 60) -> None:
        ...
        Raises:
            NorthraysError: If timeout is negative; If Sandbox fails to start or times out
        """
    ...
    async def stop(self, sandbox: AsyncSandbox, timeout: float = 60) -> None:
        ...
        Raises:
            NorthraysError: If timeout is negative; If Sandbox fails to stop or times out
        """
