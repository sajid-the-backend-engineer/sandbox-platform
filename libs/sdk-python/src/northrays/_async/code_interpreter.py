...
from northrays_toolbox_api_client_async import CreateContextRequest, InterpreterApi, InterpreterContext

from .._utils.errors import intercept_errors
from ..common.code_interpreter import ExecutionError, ExecutionResult, OutputMessage
from ..common.errors import NorthraysConnectionError, NorthraysTimeoutError
...
class AsyncCodeInterpreter:
    ...
    async def run_code(
        ...
        *,
        ...
        Raises:
            NorthraysTimeoutError: If execution times out.
            NorthraysError: If execution fails due to communication or other SDK errors.
        ...
        try:
            ...
            while True:
                ...
                if msg.type == aiohttp.WSMsgType.ERROR:
                    exc = ws.exception()
                    raise NorthraysConnectionError(f"WebSocket error: {exc}") from exc
        finally:
            if not ws.closed:
                _ = await ws.close()
    ...
    async def create_context(
        ...
        Raises:
            NorthraysError: If context creation fails.
    ...
    async def list_contexts(self) -> list[InterpreterContext]:
        ...
        Raises:
            NorthraysError: If listing fails.
    ...
    async def delete_context(self, context: InterpreterContext) -> None:
        ...
        Raises:
            NorthraysError: If deletion fails or context not found.
    ...
    def _maybe_raise_from_ws_close(self, close_code: int | None, close_reason: str | None) -> None:
        """Translate a websocket close into a Northrays error when it indicates failure.
        ...
        raise NorthraysConnectionError(detail)
