...
from northrays_api_client_async import BuildInfo, CreateSandboxSnapshot, ForkSandbox, PortPreviewUrl, ResizeSandbox
from northrays_api_client_async import Sandbox as SandboxDto
...
from northrays_toolbox_api_client_async import (
    ApiClient,
    ComputerUseApi,
    FileSystemApi,
    GitApi,
...
from ..common.northrays import CODE_TOOLBOX_LANGUAGE_LABEL
from ..common.errors import NorthraysError, NorthraysNotFoundError, NorthraysValidationError
...
class AsyncSandbox(SandboxDto):
    ...
    Attributes:
        ...
        snapshot (str | None): Northrays snapshot used to create the Sandbox.
    ...
    async def recover(self, timeout: float | None = 60):
        ...
        Raises:
            NorthraysError: If timeout is negative. If sandbox fails to recover or times out.
    ...
    async def set_autostop_interval(self, interval: int) -> None:
        ...
        Raises:
            NorthraysValidationError: If interval is negative
        ...
        """
        if interval < 0:
            raise NorthraysValidationError("Auto-stop interval must be a non-negative integer")
    ...
    async def set_auto_archive_interval(self, interval: int) -> None:
        ...
        """
        if interval < 0:
            raise NorthraysValidationError("Auto-archive interval must be a non-negative integer")
    ...
    async def update_network_settings(
        ...
        *,
        ...
        Raises:
            NorthraysValidationError: If neither argument is set.
    ...
    async def resize(self, resources: Resources, timeout: float | None = 60) -> None:
        ...
        Raises:
            NorthraysError: If hot-resize constraints are violated, disk resize is attempted on
    ...
    async def wait_for_resize_complete(
        ...
        Raises:
            NorthraysError: If timeout is negative. If resize operation times out.
        """
        check_interval = 0.1
        start_time = asyncio.get_event_loop().time()
    ...
    async def _experimental_fork(self, name: str | None = None, timeout: float | None = 60) -> "AsyncSandbox":
        ...
        Raises:
            NorthraysError: If the fork operation fails or times out.

        Example:
            ```python
            sandbox = await northrays.get("my-sandbox")
    ...
    async def _experimental_create_snapshot(self, name: str, timeout: float | None = 60) -> None:
        ...
        Raises:
            NorthraysError: If the snapshot operation fails or times out.
    ...
    async def pause(self, timeout: float = 60) -> None:
        ...
        Raises:
            NorthraysError: If timeout is negative or the operation fails/times out.
        """
        if timeout < 0:
            raise NorthraysError("Timeout must be a non-negative number")
        ...
        while self.state == "pausing":
            ...
            if 0 < remaining <= time.time() - wait_start:
                raise NorthraysError(f"Sandbox {self.id} failed to pause within {timeout} seconds")
            await asyncio.sleep(check_interval)
