...
from northrays_api_client import ApiClient, ConfigApi, Configuration, CreateBuildInfo, CreateSandbox
from northrays_api_client import GpuType as SyncGpuType
from northrays_api_client import ObjectStorageApi, SandboxApi, SandboxState, SandboxVolume, SnapshotsApi
from northrays_api_client import VolumesApi as VolumesApi
from northrays_toolbox_api_client import ApiClient as ToolboxApiClient
...
class Northrays:
    ...
    This class provides methods to create, manage, and interact with Northrays Sandboxes.
    ...
    Example:
        ...
        ```python
        northrays = Northrays()  # Uses NORTHRAYS_API_KEY, NORTHRAYS_API_URL
        ...
        async with Northrays(config) as northrays:
            sandbox = northrays.create()
    ...
    def __init__(self, config: NorthraysConfig | None = None):
        ...
        self._api_client.default_headers["X-Northrays-Source"] = "sdk-python"
        ...
        # pool is closed when the last reference (this Northrays or any Sandbox it
        # produced) goes away.
        ...
        env = env_reader or NorthraysEnvReader()
        otel_enabled = (
            (config and config.otel_enabled)
            or (config and config._experimental and config._experimental.get("otelEnabled"))
            or env.get("NORTHRAYS_OTEL_ENABLED") == "true"
            or env.get("NORTHRAYS_EXPERIMENTAL_OTEL_ENABLED") == "true"
        )
        if otel_enabled:
    ...
    def create(
        ...
        *,
        ...
        Example:
            ...
            sandbox = northrays.create(params, timeout=40)
            ```
        """
    ...
    def create(
        ...
        *,
        ...
        Example:
            ...
            ```python
            sandbox = northrays.create(CreateSandboxFromImageParams(image="debian:12.9"))
            ```
    ...
    def delete(self, sandbox: Sandbox, timeout: float = 60) -> None:
        ...
        Example:
            ...
            sandbox = northrays.create()
            # ... use sandbox ...
            northrays.delete(sandbox)  # Clean up when done
            ```
        """
    ...
    def get(self, sandbox_id_or_name: str) -> Sandbox:
        ...
        Example:
            ```python
            sandbox = northrays.get("my-sandbox-id-or-name")
            print(sandbox.state)
            ```
        """
    ...
    def list(
        ...
        Example:
            ...
            for sandbox in northrays.list(ListSandboxesQuery(labels={"env": "dev"})):
                print(sandbox.id)
            ```
        """
    ...
    @with_instrumentation(name="Northrays.list.fetch_page")
