...
from northrays_api_client import CreateBuildInfo, CreateSnapshot
from northrays_api_client import GpuType as SyncGpuType
from northrays_api_client import ObjectStorageApi
from northrays_api_client import SandboxClass as SyncSandboxClass
from northrays_api_client import SnapshotDto, SnapshotsApi, SnapshotState

from .._utils.errors import intercept_errors
from .._utils.otel_decorator import with_instrumentation
from .._utils.stream import process_streaming_response
from .._utils.timeout import with_timeout
from ..common.errors import NorthraysError, NorthraysValidationError
...
class SnapshotService:
    ...
    def list(self, page: int | None = None, limit: int | None = None) -> PaginatedSnapshots:
        ...
        if page is not None and page < 1:
            raise NorthraysValidationError("page must be a positive integer")
    ...
    def delete(self, snapshot: Snapshot) -> None:
        ...
        Example:
            ```python
            northrays = Northrays()
            snapshot = northrays.snapshot.get("test-snapshot")
    ...
    def get(self, name: str) -> Snapshot:
        ...
        Example:
            ```python
            northrays = Northrays()
            snapshot = northrays.snapshot.get("test-snapshot-name")
            print(f"{snapshot.name} ({snapshot.image_name})")
            ```
        """
