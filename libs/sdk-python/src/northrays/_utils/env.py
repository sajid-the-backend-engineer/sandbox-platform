...
class NorthraysEnvReader:
    """Reads NORTHRAYS_* env vars on demand without polluting os.environ.
    ...
    def get(self, name: str) -> str | None:
        if not name.startswith("NORTHRAYS_"):
            raise ValueError(f"NorthraysEnvReader: variable name must start with 'NORTHRAYS_', got '{name}'")
        # 1. Runtime env
    ...
    def _load(path: str) -> dict[str, str]:
        ...
        return {k: v for k, v in parsed.items() if k.startswith("NORTHRAYS_") and v is not None}
