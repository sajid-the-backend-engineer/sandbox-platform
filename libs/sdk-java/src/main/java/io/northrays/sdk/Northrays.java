...
public class Northrays implements AutoCloseable {
     ...
     *
     * <p>Reads {@code NORTHRAYS_API_KEY}, {@code NORTHRAYS_API_URL}, and {@code NORTHRAYS_TARGET}.
     *
     * @throws NorthraysException if required authentication is missing
     */
    public Northrays() {
        this(new NorthraysConfig.Builder()
                .apiKey(System.getenv("NORTHRAYS_API_KEY"))
                .apiUrl(envOrDefault("NORTHRAYS_API_URL", "https://app.northrays.com/api"))
                .target(System.getenv("NORTHRAYS_TARGET"))
                .build());
    }
     ...
     * @throws NorthraysException if configuration is invalid or missing credentials
     */
    public Northrays(NorthraysConfig config) {
        if (config == null || config.getApiKey() == null || config.getApiKey().isEmpty()) {
            throw new NorthraysException(
                    "Authentication required: set NORTHRAYS_API_KEY environment variable or pass apiKey in NorthraysConfig");
        }
        this.config = config;
        this.apiClient = createMainApiClient(config);
        this.sandboxApi = new SandboxApi(apiClient);
        this.snapshot = new SnapshotService(new io.northrays.api.client.api.SnapshotsApi(apiClient), apiClient.getHttpClient(), config.getApiKey());
        this.volume = new VolumeService(new io.northrays.api.client.api.VolumesApi(apiClient));
    }
     ...
     * @throws NorthraysException if creation or startup fails
     */
     ...
     * @return created and started {@link Sandbox}
     * @throws NorthraysException if creation fails or the Sandbox does not start in time
     */
    public Sandbox create(CreateSandboxFromSnapshotParams params, long timeoutSeconds) {
        ...
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.createSandbox(body, null));
     ...
     * @throws NorthraysException if the Sandbox is not found or request fails
     */
    public Sandbox get(String sandboxIdOrName) {
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.getSandbox(sandboxIdOrName, null, null));
     ...
     * {@code Northrays.list}" on {@link Sandbox} (env, networkBlockAll, networkAllowList, volumes,
     ...
     *
     * <pre>{@code
     * try (Stream<Sandbox> stream = northrays.listStream(query)) {
    ...
    PageResult fetchSandboxPage(ListSandboxesQuery query, String cursor) {
        ...
        String id = null;
        String name = null;
        List<io.northrays.api.client.model.SandboxState> states = null;
        ...
        io.northrays.api.client.model.SandboxListSortField sort = null;
        io.northrays.api.client.model.SandboxListSortDirection order = null;
        ...
        if (query != null) {
            ...
            if (query.getStates() != null) {
                ...
                for (io.northrays.sdk.model.SandboxState s : query.getStates()) {
        ...
        final List<io.northrays.api.client.model.SandboxState> fStates = states;
        ...
        final io.northrays.api.client.model.SandboxListSortField fSort = sort;
        final io.northrays.api.client.model.SandboxListSortDirection fOrder = order;

        io.northrays.api.client.model.ListSandboxesResponse result = ExceptionMapper.callMain(() -> sandboxApi.listSandboxes(
                null,
                fCursor,
                fLimitVal,
                fId,
        ...
        if (result != null && result.getItems() != null) {
            for (io.northrays.api.client.model.SandboxListItem item : result.getItems()) {
    ...
    /**
     * Cursor-based iterator that lazily pulls pages from the Northrays API.
     *
    ...
    private static final class SandboxIterator implements Iterator<Sandbox> {
        private final Northrays northrays;
        ...
        SandboxIterator(Northrays northrays, ListSandboxesQuery query) {
            this.northrays = northrays;
            this.query = query;
        }
        ...
        private void advanceIfNeeded() {
            while ((page == null || pageIndex >= page.size()) && !exhausted) {
                ...
                PageResult result = northrays.fetchSandboxPage(query, cursor);
    ...
    private CreateSandbox baseSandboxBody(io.northrays.sdk.model.CreateSandboxParams params) {
        ...
        if (params.getVolumes() != null) {
            ...
            for (io.northrays.sdk.model.VolumeMount mount : params.getVolumes()) {
    ...
    private io.northrays.api.client.ApiClient createMainApiClient(NorthraysConfig cfg) {
        io.northrays.api.client.ApiClient client = new io.northrays.api.client.ApiClient();
    ...
    private void ensureOauth2AuthEntry(io.northrays.api.client.ApiClient client) {
        if (client.getAuthentications().containsKey("oauth2")) {
            return;
        }
        try {
            Field authField = io.northrays.api.client.ApiClient.class.getDeclaredField("authentications");
            authField.setAccessible(true);
            Map<String, io.northrays.api.client.auth.Authentication> auths =
                    new HashMap<String, io.northrays.api.client.auth.Authentication>(
                            (Map<String, io.northrays.api.client.auth.Authentication>) authField.get(client)
                    );
            auths.put("oauth2", (q, h, c, p, m, u) -> {
            });
            authField.set(client, Collections.unmodifiableMap(auths));
        } catch (ReflectiveOperationException e) {
            throw new NorthraysException("Failed to initialize authentication", e);
        }
    }
    ...
    private String toJson(Object value) {
        try {
            ...
            throw new NorthraysException("Failed to serialize JSON", e);
        }
    }
        ...
        while (true) {
            if (timeoutSeconds > 0) {
                ...
                if (elapsed > timeoutSeconds) {
                    throw new NorthraysException("Sandbox build pending for more than " + timeoutSeconds + " seconds");
                }
            }
            io.northrays.api.client.model.Sandbox s = ExceptionMapper.callMain(() -> sandboxApi.getSandbox(sandboxId, null, null));
    ...
    private void streamSandboxBuildLogs(String sandboxId, java.util.function.Consumer<String> onLog, long timeoutSeconds, long startTime) {
        io.northrays.api.client.model.Url logsUrl = ExceptionMapper.callMain(() -> sandboxApi.getBuildLogsUrl(sandboxId, null));
