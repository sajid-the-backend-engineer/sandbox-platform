...
public class PtyHandle {
     ...
     * @throws NorthraysException if connection fails or times out
     */
    public void waitForConnection(long timeoutSeconds) {
        ...
        try {
            ...
            if (!ready) {
                throw new NorthraysException("Timed out waiting for PTY connection");
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            throw new NorthraysException("Interrupted while waiting for PTY connection", e);
        }

        if (error != null && !error.isEmpty()) {
            throw new NorthraysException("PTY connection failed: " + error, failure);
        }
        if (!connectionEstablished) {
            throw new NorthraysException("PTY connection was not established");
        }
    }
     ...
     * @throws NorthraysException if sending fails
     */
    public void sendInput(String data) {
        if (!ws.send(data)) {
            throw new NorthraysException("Failed to send PTY input");
        }
    }
    ...
    public void sendInput(byte[] data) {
        if (!ws.send(ByteString.of(data))) {
            throw new NorthraysException("Failed to send PTY binary input");
        }
    }
     ...
     * @throws NorthraysException if interrupted while waiting
     */
    public PtyResult waitForExit() {
        try {
            ...
            Thread.currentThread().interrupt();
            throw new NorthraysException("Interrupted while waiting for PTY exit", e);
        }
    }
