...
import io.northrays.api.client.model.BuildInfo;
...
import io.northrays.api.client.model.SandboxListItem;
import io.northrays.api.client.model.SandboxVolume;
...
/**
 * Represents a Northrays Sandbox instance.
 *
...
public class Sandbox {
    ...
    private final NorthraysConfig config;
    private final io.northrays.toolbox.client.ApiClient toolboxApiClient;
    private final io.northrays.toolbox.client.api.InfoApi infoApi;
    private final String apiKey;

    // Fields shared by both io.northrays.api.client.model.Sandbox and SandboxListItem.
    private String id;
    private String name;
    ...
    // Fields only present on the full Sandbox DTO; not populated by Northrays.list() —
    ...
    Sandbox(SandboxApi sandboxApi, NorthraysConfig config, io.northrays.api.client.model.Sandbox data) {
        this.sandboxApi = sandboxApi;
        this.config = config;
        this.apiKey = config.getApiKey();
        populateFromDTO(data);
        this.toolboxApiClient = buildToolboxApiClient(sandboxApi, config);
        this.infoApi = new io.northrays.toolbox.client.api.InfoApi(toolboxApiClient);
        this.process = new Process(new io.northrays.toolbox.client.api.ProcessApi(toolboxApiClient), this);
        this.fs = new FileSystem(new io.northrays.toolbox.client.api.FileSystemApi(toolboxApiClient));
        this.git = new Git(new io.northrays.toolbox.client.api.GitApi(toolboxApiClient));
        this.computerUse = new ComputerUse(new io.northrays.toolbox.client.api.ComputerUseApi(toolboxApiClient));
        this.codeInterpreter = new CodeInterpreter(new io.northrays.toolbox.client.api.InterpreterApi(toolboxApiClient), this);
    }

    Sandbox(SandboxApi sandboxApi, NorthraysConfig config, SandboxListItem data) {
        this.sandboxApi = sandboxApi;
        this.config = config;
        this.apiKey = config.getApiKey();
        populateFromDTO(data);
     ...
     */
    private io.northrays.toolbox.client.ApiClient buildToolboxApiClient(SandboxApi sandboxApi, NorthraysConfig config) {
        ...
        io.northrays.toolbox.client.ApiClient client = new io.northrays.toolbox.client.ApiClient();
        client.setBasePath(toolboxBase);
        String sdkVersion = Northrays.class.getPackage().getImplementationVersion();
        if (sdkVersion == null) sdkVersion = "dev";
        client.addDefaultHeader("Authorization", "Bearer " + config.getApiKey());
        client.addDefaultHeader("X-Northrays-Source", "sdk-java");
        client.addDefaultHeader("X-Northrays-SDK-Version", sdkVersion);
    ...
    public LspServer createLspServer(String languageId, String pathToProject) {
        return new LspServer(new io.northrays.toolbox.client.api.LspApi(toolboxApiClient));
    }

    String getLanguage() {
        String lang = "python";
        if (labels != null && labels.containsKey(Northrays.CODE_TOOLBOX_LANGUAGE_LABEL)) {
            lang = labels.get(Northrays.CODE_TOOLBOX_LANGUAGE_LABEL);
        }
        return lang;
    }
     ...
     *
     * @throws NorthraysException if the Sandbox fails to start
     */
    public void start() {
        start(60);
    }
     ...
     * @throws NorthraysException if start fails or times out
     */
    public void start(long timeoutSeconds) {
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.startSandbox(id, null));
        if (response != null) {
     ...
     *
     * @throws NorthraysException if the Sandbox fails to stop
     */
    public void stop() {
        stop(60);
    }
     ...
     *
     * @param timeoutSeconds maximum seconds to wait; {@code 0} disables timeout
     * @throws NorthraysException if stop fails or times out
     */
     ...
     *
     * @param timeoutSeconds maximum seconds to wait; {@code 0} disables timeout
     * @throws NorthraysException if timeout is invalid, state becomes error, or timeout expires
     */
    public void waitUntilStopped(long timeoutSeconds) {
        if (timeoutSeconds < 0) {
            throw new NorthraysException("Timeout must be non-negative");
        }
        ...
        while (!"stopped".equalsIgnoreCase(state) && !"destroyed".equalsIgnoreCase(state)) {
            ...
            if ("error".equalsIgnoreCase(state)) {
                throw new NorthraysException("Sandbox entered error state while stopping");
            }
            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NorthraysException("Sandbox failed to stop before timeout");
            }
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new NorthraysException("Interrupted while waiting for sandbox stop", e);
            }
        }
    }
     ...
     *
     * @throws NorthraysException if deletion fails
     */
    public void delete() {
        delete(60);
    }
     ...
     * @throws NorthraysException if label update fails
     */
     ...
     * @throws NorthraysException if the update fails
     */
    public void setAutostopInterval(int minutes) {
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.setAutostopInterval(id, BigDecimal.valueOf(minutes), null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
    ...
    public void setAutoArchiveInterval(int minutes) {
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.setAutoArchiveInterval(id, BigDecimal.valueOf(minutes), null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
    ...
    public void setAutoDeleteInterval(int minutes) {
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.setAutoDeleteInterval(id, BigDecimal.valueOf(minutes), null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
    ...
    public void updateNetworkSettings(UpdateSandboxNetworkSettings settings) {
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(() -> sandboxApi.updateNetworkSettings(id, settings, null));
        if (response != null) {
            populateFromDTO(response);
        }
    }
     ...
     * @throws NorthraysException if the request fails
     */
    public String getUserHomeDir() {
        io.northrays.toolbox.client.model.UserHomeDirResponse value = ExceptionMapper.callToolbox(() -> infoApi.getUserHomeDir());
    ...
    public String getWorkDir() {
        io.northrays.toolbox.client.model.WorkDirResponse value = ExceptionMapper.callToolbox(() -> infoApi.getWorkDir());
        return value == null ? "" : asString(value.getDir());
    }
     ...
     *
     * @param timeoutSeconds maximum seconds to wait; {@code 0} disables timeout
     * @throws NorthraysException if timeout is invalid, state becomes failure, or timeout expires
     */
    public void waitUntilStarted(long timeoutSeconds) {
        ...
        while (!"started".equalsIgnoreCase(state)) {
            ...
            if ("error".equalsIgnoreCase(state) || "build_failed".equalsIgnoreCase(state)) {
                throw new NorthraysException("Sandbox entered failure state: " + state);
            }

            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NorthraysException("Sandbox failed to become started before timeout");
            }
            ...
            try {
                ...
                Thread.currentThread().interrupt();
                throw new NorthraysException("Interrupted while waiting for sandbox start", e);
            }
        }
    }
     ...
     * — including those not returned by {@link Northrays#list} — are populated.
     *
     * @throws NorthraysException if refresh fails
     */
    public void refreshData() {
        io.northrays.api.client.model.Sandbox data = ExceptionMapper.callMain(() -> sandboxApi.getSandbox(id, null, null));
        if (data != null) {
            populateFromDTO(data);
        }
    }
    ...
    /**
     * Copies fields from the full {@link io.northrays.api.client.model.Sandbox} DTO onto this instance.
     *
     ...
     */
    private void populateFromDTO(io.northrays.api.client.model.Sandbox d) {
        if (d == null) {
            return;
        }
     ...
     * @throws NorthraysException if the fork operation fails or times out
     */
    ...
    public Sandbox experimentalFork(String name, long timeoutSeconds) {
        ...
        io.northrays.api.client.model.Sandbox response = ExceptionMapper.callMain(
     ...
     * @throws NorthraysException if the snapshot operation fails
     */
    ...
    private void waitForSnapshotComplete(long timeoutSeconds) {
        ...
        while ("snapshotting".equalsIgnoreCase(state)) {
            refreshData();
            if ("error".equalsIgnoreCase(state) || "build_failed".equalsIgnoreCase(state)) {
                throw new NorthraysException("Sandbox snapshot failed with state: " + state);
            }
            if (!"snapshotting".equalsIgnoreCase(state)) {
                return;
            }
            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NorthraysException("Sandbox snapshot did not complete before timeout");
            }
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new NorthraysException("Interrupted while waiting for snapshot complete", e);
            }
        }
    }
     ...
     * Uses default timeout of 60 seconds.
     *
     * @throws NorthraysException if the pause operation fails
     */
    public void pause() throws NorthraysException {
        pause(60);
    }
     ...
     * @throws NorthraysException if timeout is negative or the operation fails/times out
     */
    public void pause(long timeoutSeconds) throws NorthraysException {
        if (timeoutSeconds < 0) {
            throw new NorthraysException("Timeout must be a non-negative number");
        }
    ...
    private void waitForPauseComplete(long timeoutSeconds) {
        ...
        while ("pausing".equalsIgnoreCase(state)) {
            refreshData();
            if ("error".equalsIgnoreCase(state) || "build_failed".equalsIgnoreCase(state)) {
                throw new NorthraysException("Sandbox pause failed with state: " + state);
            }
            if (!"pausing".equalsIgnoreCase(state)) {
                return;
            }
            if (timeoutSeconds > 0 && (System.currentTimeMillis() - startedAt) > timeoutSeconds * 1000L) {
                throw new NorthraysException("Sandbox pause did not complete before timeout");
            }
            try {
                Thread.sleep(250);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
                throw new NorthraysException("Interrupted while waiting for pause complete", e);
            }
        }
    }
    ...
    /** @return Northrays snapshot used to create this Sandbox, or {@code null} if none. */
     ...
     *
     * <p>Not returned by {@link Northrays#list}; call {@link #refreshData()} on each item to populate.
     *
    ...
    io.northrays.toolbox.client.ApiClient getToolboxApiClient() { return toolboxApiClient; }
