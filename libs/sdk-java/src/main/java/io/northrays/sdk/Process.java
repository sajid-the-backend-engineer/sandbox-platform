...
import io.northrays.sdk.model.*;
import io.northrays.sdk.exception.NorthraysException;
import io.northrays.toolbox.client.api.ProcessApi;
import io.northrays.toolbox.client.model.CodeRunRequest;
import io.northrays.toolbox.client.model.CodeRunResponse;
import io.northrays.toolbox.client.model.CreateSessionRequest;
import io.northrays.toolbox.client.model.ExecuteRequest;
import io.northrays.toolbox.client.model.PtyCreateRequest;
import io.northrays.toolbox.client.model.PtyCreateResponse;
import io.northrays.toolbox.client.model.PtyListResponse;
import io.northrays.toolbox.client.model.PtyResizeRequest;
import io.northrays.toolbox.client.model.PtySessionInfo;
import io.northrays.toolbox.client.model.SessionSendInputRequest;
...
public class Process {
     ...
     * @throws NorthraysException if execution fails
     */
    ...
    public ExecuteResponse executeCommand(String command, String cwd, Map<String, String> env, Integer timeout) {
        ...
        io.northrays.toolbox.client.model.ExecuteResponse response = ExceptionMapper.callToolbox(() -> processApi.executeCommand(request));
     ...
     * @throws NorthraysException if session creation fails
     */
     ...
     * @throws NorthraysException if retrieval fails
     */
    public Session getSession(String sessionId) {
        io.northrays.toolbox.client.model.Session session = ExceptionMapper.callToolbox(() -> processApi.getSession(sessionId));
    ...
    public Session getEntrypointSession() {
        io.northrays.toolbox.client.model.Session session = ExceptionMapper.callToolbox(processApi::getEntrypointSession);
        return toSession(session);
    }
    ...
    public SessionExecuteResponse executeSessionCommand(String sessionId, SessionExecuteRequest req) {
        io.northrays.toolbox.client.model.SessionExecuteRequest request = new io.northrays.toolbox.client.model.SessionExecuteRequest()
                .command(req.getCommand())
                .runAsync(req.getRunAsync());
        io.northrays.toolbox.client.model.SessionExecuteResponse response = ExceptionMapper.callToolbox(() -> processApi.sessionExecuteCommand(sessionId, request));
    ...
    public Command getSessionCommand(String sessionId, String commandId) {
        io.northrays.toolbox.client.model.Command command = ExceptionMapper.callToolbox(() -> processApi.getSessionCommand(sessionId, commandId));
     ...
     * @throws NorthraysException if streaming fails
     */
     ...
     * @throws NorthraysException if sending input fails
     */
     ...
     *
     * @return session list
     * @throws NorthraysException if listing fails
     */
    public List<Session> listSessions() {
        List<io.northrays.toolbox.client.model.Session> sessions = ExceptionMapper.callToolbox(processApi::listSessions);
        List<Session> output = new ArrayList<Session>();
        if (sessions != null) {
            for (io.northrays.toolbox.client.model.Session session : sessions) {
     ...
     * @throws NorthraysException if PTY session creation fails
     */
    public PtyHandle createPty(PtyCreateOptions options) {
        ...
        if (response == null || response.getSessionId() == null || response.getSessionId().isEmpty()) {
            throw new NorthraysException("Failed to create PTY session");
        }
     ...
     * @return PTY handle for streaming I/O and lifecycle operations
     * @throws NorthraysException if websocket connection setup fails
     */
     ...
     * @throws NorthraysException if resize fails
     */
     ...
     *
     * @param sessionId PTY session identifier
     * @throws NorthraysException if termination fails
     */
    ...
    private String buildPtyWebSocketUrl(String toolboxBaseUrl, String sessionId) {
        if (toolboxBaseUrl == null || toolboxBaseUrl.isEmpty()) {
            throw new NorthraysException("Toolbox base URL is not available");
        }
    ...
    private void streamDemuxedLogsViaWebSocket(String wsUrl, Consumer<String> onStdout, Consumer<String> onStderr) {
        ...
        sandbox.getToolboxApiClient().getHttpClient().newWebSocket(wsRequest, new WebSocketListener() {
            ...
            public void onFailure(WebSocket webSocket, Throwable t, Response response) {
                ...
                failure.compareAndSet(null, new NorthraysException("Log streaming failed: " + msg, t));
                doneLatch.countDown();
            }
        ...
        try {
            ...
            throw new NorthraysException("Interrupted while streaming logs", e);
        }
    ...
    private ExecuteResponse toExecuteResponse(io.northrays.toolbox.client.model.ExecuteResponse source) {
    ...
    private Session toSession(io.northrays.toolbox.client.model.Session source) {
        return new Session(source);
    }

    private SessionExecuteResponse toSessionExecuteResponse(io.northrays.toolbox.client.model.SessionExecuteResponse source) {
