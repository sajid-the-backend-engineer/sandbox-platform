...
import { ObjectStorageApi, SnapshotsApi, SnapshotState, SandboxClass, Configuration } from '@northrays/api-client'
import type { SnapshotDto, CreateSnapshot, PaginatedSnapshots as PaginatedSnapshotsDto } from '@northrays/api-client'
...
/**
 * Represents a Northrays Snapshot which is a pre-configured sandbox.
 *
...
/**
 * Represents a paginated list of Northrays Snapshots.
 *
 ...
 * If an Image instance is provided, it will be used to create a new image in Northrays.
...
/**
 * Service for managing Northrays Snapshots. Can be used to list, get, create and delete Snapshots.
 *
 * @class
 */
export class SnapshotService {
   ...
   * @example
   * const northrays = new Northrays();
   * const snapshot = await northrays.snapshot.get("snapshot-name");
   ...
   * const northrays = new Northrays();
   * const snapshot = await northrays.snapshot.get("snapshot-name");
   * await northrays.snapshot.delete(snapshot);
   ...
   * await northrays.snapshot.create({ name: 'my-snapshot', image: image }, { onLogs: console.log });
   */
  @WithInstrumentation()
  public async create(
    ...
    if (!createdSnapshot) {
      throw new NorthraysError("Failed to create snapshot. Didn't receive a snapshot from the server API.")
    }
