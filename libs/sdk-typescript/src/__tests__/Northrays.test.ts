...
// short-circuit NorthraysEnvReader to read process.env only — never the
...
jest.mock('../utils/Runtime', () => {
  ...
  return { ...actual, NorthraysEnvReader: TestEnvReader }
})
...
describe('Northrays', () => {
  beforeEach(() => {
    ...
    delete process.env.NORTHRAYS_API_KEY
    delete process.env.NORTHRAYS_JWT_TOKEN
    delete process.env.NORTHRAYS_ORGANIZATION_ID
    delete process.env.NORTHRAYS_API_URL
    delete process.env.NORTHRAYS_SERVER_URL
    delete process.env.NORTHRAYS_TARGET
  ...
  it('uses explicit api key config and builds dependent services', async () => {
    const { Northrays } = await import('../Northrays')

    const instance = new Northrays({
      apiKey: 'api-key',
      apiUrl: 'https://example.northrays.test/api',
      target: 'eu',
    })
  ...
  it('reads constructor values from env when config omitted', async () => {
    process.env.NORTHRAYS_API_KEY = 'env-key'
    process.env.NORTHRAYS_API_URL = 'https://env.northrays/api'
    process.env.NORTHRAYS_TARGET = 'us'
    ...
    expect(firstConfigArg.basePath).toBe('https://env.northrays/api')
  ...
  it('falls back to the default api url when none is provided', async () => {
    ...
    expect(firstConfigArg.basePath).toBe('https://app.northrays.com/api')
  })

  it('supports deprecated serverUrl config', async () => {
    const { Northrays } = await import('../Northrays')

    new Northrays({ apiKey: 'k', serverUrl: 'https://legacy.northrays/api', target: 'us' })

    const firstConfigArg = mockConfigurationCtor.mock.calls[0][0] as {
      basePath: string
    }

    expect(firstConfigArg.basePath).toBe('https://legacy.northrays/api')
  })

  it('reads deprecated server url from env and warns once', async () => {
    process.env.NORTHRAYS_API_KEY = 'env-key'
    process.env.NORTHRAYS_SERVER_URL = 'https://server.northrays/api'
    process.env.NORTHRAYS_TARGET = 'us'
    ...
    expect(firstConfigArg.basePath).toBe('https://server.northrays/api')
  ...
  it('supports jwt auth with organization header', async () => {
    ...
    new Northrays({
      ...
      apiUrl: 'https://jwt.northrays/api',
      target: 'us',
    })
    ...
    expect(firstConfigArg.baseOptions.headers['X-Northrays-Organization-ID']).toBe('org-1')
  })
  ...
  it('throws when no credentials are provided', async () => {
    ...
    delete process.env.NORTHRAYS_JWT_TOKEN
    delete process.env.NORTHRAYS_ORGANIZATION_ID
    expect(() => new Northrays()).toThrow('Authentication credentials not found.')
  })
  ...
  it('throws when jwt auth has no organization id', async () => {
    ...
    ).toThrow('NORTHRAYS_ORGANIZATION_ID is required when authenticating with NORTHRAYS_JWT_TOKEN.')
  })

  it('reads jwt credentials from env when config omits them', async () => {
    process.env.NORTHRAYS_JWT_TOKEN = 'env-jwt'
    process.env.NORTHRAYS_ORGANIZATION_ID = 'env-org'
    process.env.NORTHRAYS_API_URL = 'https://env-jwt.northrays/api'
    process.env.NORTHRAYS_TARGET = 'eu'
    ...
    expect(firstConfigArg.baseOptions.headers['X-Northrays-Organization-ID']).toBe('env-org')
  })

  it('throws unsupported language in create', async () => {
    const { Northrays } = await import('../Northrays')
    const instance = new Northrays({ apiKey: 'k', apiUrl: 'http://api', target: 'us' })
  ...
  it('wraps NorthraysTimeoutError from sandbox startup in create', async () => {
    const { Northrays } = await import('../Northrays')
    const { Sandbox } = await import('../Sandbox')
    const { NorthraysTimeoutError } = await import('../errors/NorthraysError')
    const instance = new Northrays({ apiKey: 'k', apiUrl: 'http://api', target: 'us' })
    ...
    ;(Sandbox as jest.Mock).mockImplementationOnce((dto: { id: string; state?: string }) => ({
      ...
      stop: jest.fn(),
      delete: jest.fn(),
      waitUntilStarted: jest.fn().mockRejectedValue(new NorthraysTimeoutError('slow start')),
      _experimental_fork: jest.fn(),
    }))
