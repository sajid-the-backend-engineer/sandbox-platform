...
import type { GpuType, SandboxVolume } from '@northrays/api-client'
...
import { NorthraysEnvReader, RUNTIME, Runtime } from './utils/Runtime'
...
/**
 * Configuration options for initializing the Northrays client.
 *
 * @interface
 * @property {string} apiKey - API key for authentication with the Northrays API
 * @property {string} jwtToken - JWT token for authentication with the Northrays API. If not set, it must be provided
 * via the environment variable `NORTHRAYS_JWT_TOKEN`, or an API key must be provided instead.
 * @property {string} organizationId - Organization ID used for JWT-based authentication. Required if a JWT token
 * is provided, and must be set either here or in the environment variable `NORTHRAYS_ORGANIZATION_ID`.
 * @property {string} apiUrl - URL of the Northrays API. Defaults to 'https://app.northrays.com/api'
 * if not set here and not set in environment variable NORTHRAYS_API_URL.
 ...
 *     target: "us"
 * };
 * const northrays = new Northrays(config);
 */
export interface NorthraysConfig {
 ...
 * Provides methods for creating, managing, and interacting with Northrays Sandboxes.
 * Can be initialized either with explicit configuration or using environment variables.
 *
 * @property {VolumeService} volume - Service for managing Northrays Volumes
 * @property {SnapshotService} snapshot - Service for managing Northrays Snapshots
 *
 * @example
 * // Using environment variables
 * // Uses NORTHRAYS_API_KEY, NORTHRAYS_API_URL, NORTHRAYS_TARGET
 ...
 *
 * @example
 * // Disposes northrays and flushes traces when done
 * await using northrays = new Northrays({
 *   otelEnabled: true,
 * });
 * @class
 */
export class Northrays implements AsyncDisposable {
  ...
  /**
   * Creates a new Northrays client instance.
   *
   * @param {NorthraysConfig} [config] - Configuration options
   * @throws {NorthraysAuthenticationError} When no credentials are provided (neither API key nor JWT token)
   * @throws {NorthraysAuthenticationError} When JWT token is provided without an organization ID
   */
  constructor(config?: NorthraysConfig) {
    ...
    let _envReader: NorthraysEnvReader | null | undefined
    const envReader = (): NorthraysEnvReader | null => {
      if (_envReader === undefined) {
        _envReader = RUNTIME !== Runtime.BROWSER ? new NorthraysEnvReader() : null
      }
      return _envReader
    }
    ...
    if (
      ...
      if (reader) {
        this.apiKey = this.apiKey || (this.jwtToken ? undefined : reader.get('NORTHRAYS_API_KEY'))
        this.jwtToken = this.jwtToken || reader.get('NORTHRAYS_JWT_TOKEN')
        this.organizationId = this.organizationId || reader.get('NORTHRAYS_ORGANIZATION_ID')
        apiUrl = apiUrl || reader.get('NORTHRAYS_API_URL') || reader.get('NORTHRAYS_SERVER_URL')
        this.target = this.target || reader.get('NORTHRAYS_TARGET')

        if (reader.get('NORTHRAYS_SERVER_URL') && !reader.get('NORTHRAYS_API_URL')) {
          console.warn(
            '[Deprecation Warning] Environment variable `NORTHRAYS_SERVER_URL` is deprecated and will be removed in future versions. Use `NORTHRAYS_API_URL` instead.',
          )
        }
      }
    }

    this.apiUrl = apiUrl || 'https://app.northrays.com/api'

    if (!this.apiKey && !this.jwtToken) {
      throw new NorthraysAuthenticationError(
        'Authentication credentials not found. Set NORTHRAYS_API_KEY, or both NORTHRAYS_JWT_TOKEN and NORTHRAYS_ORGANIZATION_ID.' +
    ...
    if (!this.apiKey) {
      ...
      orgHeader['X-Northrays-Organization-ID'] = this.organizationId
    }

    const isLegacyPackage = packageJson.name === '@northrays/sdk'
    ...
    const configuration = new Configuration({
      ...
      baseOptions: {
        headers: {
          ...
          'X-Northrays-Source': sdkLabel,
          'X-Northrays-SDK-Version': packageJson.version,
    ...
    const axiosInstance = Northrays.createAxiosInstance()
    ...
    const otelEnabled =
      ...
      env?.get('NORTHRAYS_OTEL_ENABLED') === 'true' ||
      env?.get('NORTHRAYS_EXPERIMENTAL_OTEL_ENABLED') === 'true'
    ...
    this.otelSdk = new NodeSDK({
      resource: resourceFromAttributes({
        ...
        [ATTR_SERVICE_NAME]: 'northrays-typescript-sdk',
      }),
      instrumentations: [
   ...
   * };
   * const sandbox = await northrays.create(params, { timeout: 100 });
   */
  public async create(params?: CreateSandboxFromSnapshotParams, options?: { timeout?: number }): Promise<Sandbox>
  /**
   * Creates Sandboxes from specified image available on some registry or declarative Northrays Image. You can specify various parameters,
   * including resources, language, image, environment variables, and volumes. Northrays creates snapshot from
   ...
   *
   * @example
   * const sandbox = await northrays.create({ image: 'debian:12.9' }, { timeout: 90, onSnapshotCreateLogs: console.log });
   *
   * @example
   * // Create a custom sandbox
   ...
   *     autoDeleteInterval: 120
   * };
   * const sandbox = await northrays.create(params, { timeout: 100, onSnapshotCreateLogs: console.log });
   */
  public async create(
  ...
  public async create(
    ...
    try {
        ...
        while (sandboxInstance.state === SandboxState.PENDING_BUILD) {
          if (options.timeout) {
            ...
            if (elapsed > options.timeout) {
              throw new NorthraysTimeoutError(
      ...
      if (error instanceof NorthraysTimeoutError) {
        ...
        throw new NorthraysTimeoutError(errMsg, error.statusCode, error.headers, error.errorCode)
      }
   ...
   *
   * @example
   * const sandbox = await northrays.get('my-sandbox-id-or-name');
   ...
   *
   * @example
   * for await (const sandbox of northrays.list({ labels: { env: 'dev' } })) {
    ...
    async function* generator(): AsyncGenerator<Sandbox> {
      ...
      while (true) {
        ...
        for (const sandbox of response.data.items) {
          ...
          yield new Sandbox(sandbox, structuredClone(clientConfig), Northrays.createAxiosInstance(), sandboxApi)
        }
   ...
   * await northrays.start(sandbox, 60);
   */
  @WithInstrumentation()
   ...
   * @example
   * const sandbox = await northrays.get('my-sandbox-id');
   * const forked = await northrays._experimental_fork(sandbox, { name: 'my-fork' });
    ...
    axiosInstance.interceptors.response.use(
      ...
      (error) => {
        if (error instanceof AxiosError) {
          throw createAxiosNorthraysError(error)
        }

        throw new NorthraysError(error instanceof Error ? error.message : String(error))
      },
    )
